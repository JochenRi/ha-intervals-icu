...
/* ── state is never colour alone (WCAG 1.4.1) ──────────────────────────── */
{
  ...
  for (const word of ["rot", "grün", "gelb", "keine Daten"]) contains(html, word, "barrierefreiheit");
...
{
  const all = [p.rHeute(rd, days, load), p.rKalender(days), p.rFitness(F.pmc(days), 182),
  ...
  contains(p.rHeute(rd, days, load), "bval", "gestaltung: Bullet-Graph fehlt");
}

/* ── fold-outs are independent ─────────────────────────────────────────── */
{
  const html = p.rHeute(rd, days, load) + p.rBelastung(load);
  ...
  ok(det >= 10, `gestaltung: nur ${det} Aufklappfelder - Quellen stehen als Textwand daneben`);
}

/* ── one lead figure per view ──────────────────────────────────────────── */
{
  const views = { dfa: p.rDfa(thr, "all"), heute: p.rHeute(rd, days, load) };
  ok((views.dfa.match(/lead1/g) || []).length === 1, "gestaltung: DFA ohne genau eine Leitzahl");
  ok((views.heute.match(/class="ring"/g) || []).length === 1, "gestaltung: Heute ohne genau eine Leitanzeige");
}
...
/* ── sources stay attached to every derived number ─────────────────────── */
{
  ...
  const heute = p.rHeute(rd, days, load);
