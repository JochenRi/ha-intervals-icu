...
**Stand:** 26.09.2026 · **Version:** 0.74.4 · **Status:** produktiv auf HEIMDALL,
...
9 Familien · 24 Testdateien mit **9.711** gezählten Einzelprüfungen · 100 Tags (`git tag | wc -l`).
...
**Einundvierzigster Fall (0.66.1, 23.09.2026, Karte F1.11 — bestätigt am Code und an einer
Fixture, 47 Prüfungen): ein Lesebefehl hat gelöscht, was er nicht prüfen konnte.**

`websocket_laps` prüft beim Öffnen einer markierten Fahrt gegen die live geholten Runden, ob
die Marken noch sitzen (`section_marks.drift`). `drift` kennt drei Gründe: `lap_count`,
`section_moved` — und `laps_missing`, „die Runden sind nicht da". Der Aufrufer behandelte
alle drei gleich: `if stale: drop_hours(...)`. Liefert Intervals für eine Fahrt keine
Runden (Feldname geändert, Antwort leer, Fahrt ohne Intervalle), fiel beim **bloßen
Anklicken** die Messung **aller** Familien der Fahrt, mit dem Grund `moved`, den niemand
festgestellt hatte, und mit Speichern. Sieben Wege führten dorthin: Aktivitätenliste,
Kalender-Chip, Adresse, die Fahrtenlinks in Ermüdungs- und Blockkacheln, im Nachweis einer
Einheit, in der Kurvenliste und im DFA-Reiter. Der Messweg daneben (`measure_section_marks`)
steigt bei leeren Runden **vor** dem Messen aus — die beiden Wege behandelten „keine
Runden" gegensätzlich. Und der Ausweg im Panel („Zuordnung auf den neuen Stand setzen")
hätte bei diesem Grund gegen leere Runden neu verankert.

**Behoben in 0.66.1 (R1, Haltung i):** die Löschung ist auf die **festgestellten** Gründe
beschränkt — `DRIFT_FOUND = {lap_count, section_moved}` in `section_marks`, und der Handler
fragt diese Menge. Bei `laps_missing` bleibt die Messung stehen, der Befund reist wie bisher
mit, das Panel sagt „konnte nicht geprüft werden" ohne Bestätigen-Knopf, und der Satz an der
Einheit sagt, dass die Messung steht und **nicht verwendet** wird, bis die Runden da sind
(`usable_hours` gibt bei jedem Driftbefund `None` — behalten heißt nicht benutzen; nicht
rechnen ist nicht dasselbe wie löschen).

**Warum der Prüfstand es nicht sah:** `drift([]) == "laps_missing"` war geprüft
(test_section_marks), `drop_hours` mit `section_moved` war geprüft — die **Entscheidung
im Handler** hatte keinen Fall mit leeren Runden. Funktion geprüft, Aufrufer nicht
(Bauregel 3; Karte 5, Klasse P1). Die neue Prüfung stand **vor** dem Bau rot an 0.66.0
(vier Zeilen), die Gegenproben grün; nach dem Bau grün; mit zurückgenommener Reparatur
(Dateikopie) wieder rot — vier in test_handlers, drei in test_panel_fixes.

**Lehre:** ein Befund, der „ich weiß es nicht" heißt, ist kein Befund. Wer eine Prüfung nicht
ausführen kann, hat nichts festgestellt und darf nichts fallen lassen — er meldet, und die
Verwendung bleibt gesperrt.

---

**Zweiundvierzigster Fall (0.66.2, 23.09.2026, Karte F1.8 — bestätigt am Code und an einer
Fixture): ein Haken in Familie A hat die Messung von Familie B gelöscht.**

`section_marks._write` schrieb bei jedem Setzen und jeder Rücknahme einer Marke
`"measure": {}` — für den **ganzen** Eintrag. Der Kommentar begründete es mit „ändert den
Ausschnitt"; die Maske ist aber seit 0.54.1 familienrein, eine Grundlagen-Messung sitzt nicht
auf einem SweetSpot-Abschnitt. Auf den zwei Fahrten mit zwei Familien (05.08., 20.08.) nahm
jeder Haken in der einen Familie der Kurve einen Punkt aus der anderen, mit `lost = changed`
und Speichern. Nebenbefund F1.8b: dieselbe Marke noch einmal setzen (über den WebSocket-Befehl
möglich) leerte ebenfalls und speicherte.

**Behoben in 0.66.2 (R2):** `_write` vergleicht die Markenliste je Familie vorher/nachher.
Unveränderte Familien behalten ihr Fach, veränderte verlieren es mit Grund; ändert sich keine,
kommt der alte Eintrag zurück und `set_section_mark` speichert nicht (J7, vorher/nachher
verglichen). **Sichtbar gemacht, nicht gelöst (F1.10):** `measured_at` und `v` gelten weiter
für den ganzen Eintrag — nach einem Teil-Erhalt stimmt das Messdatum nur noch für die
behaltene Familie. Das steht jetzt im Docstring, statt sich hinter dem Datum zu verstecken.

**Warum der Prüfstand es nicht sah:** `test_section_marks.py:218-232` prüfte „die Messmarke
nimmt die Zahlen, Marken und Anker bleiben" — an **einer** Familie. Der Wächter schrieb das
Verhalten fest, statt den Fall mit zwei Familien zu stellen (Karte 5, Klasse P1). Neue Prüfung
vor dem Bau rot an 0.66.1 (vier Zeilen, plus zwei im Handler), nach dem Bau grün, unter drei
Mutationen wieder rot.

**Lehre:** „ändert den Ausschnitt" ist ein Satz über eine Familie. Wer ihn auf den Eintrag
anwendet, löscht, was er nicht angefasst hat — und eine Fixture mit einer Familie kann das
nicht sehen.

---

**Dreiundvierzigster Fall (0.66.3, 23.09.2026, Karte F1.5 / Sollzustand S8): drei Zähler
verglichen noch auf „ungleich" — der Rückweg hätte `dfa` und die Stufentests gelöscht.**

Der vierzigste Fall hat die Regel „ÄLTER statt UNGLEICH" für `section_marks.v` eingeführt und
in seiner Tabelle fünf weitere Stellen derselben Bauart **gelistet, keine geprüft**. Drei davon
verglichen bis 0.66.2 weiter auf ungleich: `importer.drop_outdated_dfa` (`dfa_version`),
`importer.needs_activity_refetch` (`fields_version`) und `ramp_tests.migrate` (`v`). Ein
HACS-Downgrade — seit dem 19.09. der vorgesehene Rückweg — hätte beim ersten Laden den ganzen
`dfa`-Block (Anker, Stunden, Blöcke aller Fahrten) verworfen und jedes Stufentest-Ergebnis auf
„neu zu messen" gesetzt; `mark_activities_current` hätte die Marke danach heruntergeschrieben,
sodass der nächste echte Bump alles ein zweites Mal geholt hätte.

**Behoben in 0.66.3:** ein Modul `versions.py` mit **einer** Regel — `is_older` (fehlend, None,
Unsinn = 0 = älter als alles) und `keep_newest` (nie nach unten). Alle vier Zähler lesen sie:
die drei genannten Stellen und `section_marks` (dessen `_version_of` und die zwei Vergleiche
sind jetzt Durchgriffe). Eine Mutation der Regel auf `!=` lässt sieben Prüfungen in vier
Dateien fallen; vorher fiel keine außerhalb der Marken.

**Was das nicht löst und so gewollt ist:** nach einem Downgrade stehen `dfa`-Zeilen der neueren
Rechnung neben Zeilen der älteren, bis der nächste Bump alles neu holt — dieselbe Mischung, die
0.63.3 für die Marken in Kauf genommen hat. Daten behalten schlägt Daten reinhalten.

**Warum der Prüfstand es nicht sah:** `test_import.py:235-239` prüfte „ältere Version wird
verworfen" — nie „neuere bleibt". Der 0.63.3-Wächter war nur für die Marken verallgemeinert,
nicht für die Zähler daneben (Karte 5, Klasse P1: Wächter nicht verallgemeinert). Neue
Prüfungen vor dem Bau rot an 0.66.2 (sechs in test_import, zwei in test_ramp), nach dem Bau grün,
unter zwei Mutationen (`!=`, `keep_newest → current`) wieder rot.

**Lehre:** eine Tabelle „fünf weitere Stellen, keine geprüft" ist eine Liste offener
Löschungen. Eine Regel, die an einer Stelle richtig ist, ist an den anderen erst richtig, wenn
alle sie **lesen** statt sie nachzubauen.

---

**Vierundvierzigster Fall (0.66.3, 23.09.2026, Karte F1.6 / W1.6): der Abgleich räumte drei
von sechs Blöcken — eine in Intervals gelöschte Fahrt lebte über Marke und Stufentest weiter.**

`reconcile.apply` hieß im Docstring „all three tidy-up sites" und räumte `activities`, `dfa`,
`unavailable`. Id-geschlüsselt sind seit 0.5x sechs Blöcke. Für eine markierte Fahrt ohne
Aktivität baut `blocks._marked_sessions` ausdrücklich eine Ersatzzeile aus dem Markendatum
(`blocks.py:107`), und `ramp_tests.latest` fragt die Aktivität gar nicht: eine drüben gelöschte
Fahrt rechnete weiter in Blockreihe, Steuerung und — bis 0.66.3 — in der Blockleiter der
Umrechnung. `dfa_failed` blieb ebenfalls stehen.

**Behoben in 0.66.3:** `ID_BLOCKS` als **eine** Liste der id-geschlüsselten Blöcke in
`reconcile`; `apply` läuft über sie und zählt je Block. Weil Marken und Stufentests
Athleteneingaben sind, sagt `plan` sie **vorher** an (`marks`, `ramp` je fehlender Einheit, wie
bisher `dfa`), der Dialog zeigt „inkl. Markierung / inkl. Stufentest" im Befund und beziffert
sie im Vollzug. Nichts fällt still. Ein Wächter hält das Register fest; wer einen Block mit
Aktivitäts-ids anlegt, muss ihn eintragen.

**Warum der Prüfstand es nicht sah:** `test_reconcile` prüfte je Stelle „entfernt" und
„unberührt" — für die drei, die der Docstring nannte. Der Prüfstand fragte nie, **welche**
Blöcke id-geschlüsselt sind (Karte 5, Klasse P2: der Wächter so eng wie der Satz darüber).
Neue Prüfungen rot vor dem Bau (neun im Modul, vier im Panel), nach dem Bau grün, unter drei
Mutationen (Register auf zwei, Ansage entfernt, Bezifferung entfernt) wieder rot.

**Lehre:** „alle drei" ist eine Zahl aus dem Tag, an dem der Satz geschrieben wurde. Eine
Aufräumstelle zählt aus einem Register, nicht aus einem Docstring.

---

**Fünfundvierzigster Fall (0.66.3, 23.09.2026, Karte F1.9): nach jedem Rechenschalter standen
die Blockfamilien im Sammellauf — für eine Neumessung, die dieselbe Zahl liefert.**

`dfa_hours` liest die Leistung bei alpha über die Achse, die der Rechenschalter wählt;
`dfa_blocks` bildet Mediane über Rohwatt und kennt kein Fenster. Der Messweg stempelte `w`
trotzdem für jede Familie, und `pending_remeasure` verglich jede Familie gegen die
Schalterstellung. Folge: nach dem Umlegen meldete die Leiste VO2max-, SweetSpot- und
Tempo-Marken als „andere Wattachse", der Sammellauf holte ihre Ströme neu und schrieb dieselben
Blockmediane zurück. Und `test_section_marks` **schrieb das fest** („bei der anderen Achse ist es
umgekehrt": E und F erwartet).

**Behoben in 0.66.3:** `WINDOW_FAMILIES = ("endurance",)` und `window_matters(fam)` in
`section_marks` — die eine Stelle, die sagt, wessen Messung an der Achse hängt.
`pending_remeasure` fragt sie; `fatigue._marked_rides` liest sie statt „endurance" zu wissen.
Das Feld `w` bleibt an jeder Messung stehen (es sagt, unter welcher Stellung gemessen wurde);
gefragt wird es nur, wo es zählt.

**Warum der Prüfstand es nicht sah:** er sah es — und hielt es für richtig. Der Wächter aus
0.65.0 prüfte „Blockmessung zählt als gemessen" und daneben „bei der anderen Achse ist es
umgekehrt" mit E und F in der Liste. Zwei Zusicherungen, die zusammen den Leerlauf beschreiben
(Karte 5, Klasse P1: die Fixture stellte den Fall, der Wächter nannte ihn Soll). Neue Prüfung rot
vor dem Bau (vier Zeilen), nach dem Bau grün, unter der Mutation `window_matters → True`
wieder rot.

**Lehre:** eine Gegenprobe, die den Ist-Zustand als Erwartung trägt, ist keine Gegenprobe. Vor
jedem „so ist es heute, also ist es richtig" gehört die Frage, ob die Zahl an der Stelle
überhaupt etwas messen kann.

---

**Sechsundvierzigster Fall (0.66.3, 23.09.2026, Karte F1.14): der Satz „an deinen Marken
vorbei" hing am falschen Schalter.**

`not_active.blocks` fiel weg, sobald `steering_v2` an stand (seit 0.60.0). Ob die Blockreihe
die Marken liest, entscheidet aber `blocks_from_marks`. In den zwei Kreuzstellungen sagte die
Kachel das Gegenteil der Lage: Blockschalter an, Steuerung aus — „an deinen Marken vorbei" neben
einer Reihe, die auf ihnen rechnete; Blockschalter aus, Steuerung an — kein Satz, obwohl die
Marken nichts taten. Behoben: der Satz hängt an `blocks_lib.blocks_from_marks`. Prüfung mit
allen vier Kreuzstellungen, zwei rot vor dem Bau; Mutation zurück auf `steering_on` → zwei rot.
Der Wächter aus 0.60.0 prüfte nur eine Achse (Klasse P1).

---

**Siebenundvierzigster Fall (0.66.3, 23.09.2026, Michael-Befund): die App war für einen
Athleten gebaut — ein zweiter sah im Kachelkopf Johannes' Vorgabe.**

`const.py` trug `STEERING_ANCHOR_W = {sweetspot: 190, vo2max: 250}` und
`STEERING_ANCHOR_DATE = "2026-09-17"`: Johannes' Startwerte aus seinen vier SweetSpot-Einheiten
(189/190/192/194 W) und sein Stichtag, fest im Code. Michael legte den Wattvorgabe-Schalter um
und sah im Kopf der Kachel 190 und 250 W, darüber seine eigenen Einheiten — und keine davon
zählte, weil alle vor dem 17.09.2026 lagen. Das ist eine **Vorgabe**, nach der er gefahren
wäre. An einer Fixture mit einem erfundenen zweiten Athleten (150/210 W, Einheiten vor und nach
dem Stichtag) nachgestellt: der alte Code zeigt 190/250, `n_since` 2, `moves` 0.

**Behoben in 0.66.3:** der Startwert entsteht **je Athlet im Archiv** (`settings.steering_anchor`,
je Familie `{w, date, source, units}`) — beim Einschalten der Steuerung aus dem Median der
letzten vier eigenen Einheiten (ab drei), später beim Messen oder Sync, sobald eine Familie
genug hat; der Stichtag ist der Tag seiner Entstehung; Ausschalten löscht ihn nicht. Fehlt er,
sagt die Kachel „noch kein Startwert — n von 3" und zeigt keine Vorgabe. `compare` trägt ohne
Vorgabe kein Delta mehr — damit ist **F2.1** (Absturz des `blocks`-Befehls) mit erledigt.

**Die Übernahme für den ersten Athleten:** sein Startwert **galt** seit 0.62.0 und darf durch die
Reparatur nicht neu entstehen (er läge sonst bei 191/244 statt 190/250). `migrate_legacy_anchor`
schreibt ihn einmal ins Archiv — unter **zwei** Bedingungen: der Schalter steht an, **und** der
Code-Startwert passt zum Bestand (eigener Startwert aus den Einheiten vor dem 17.09. innerhalb
von `LEGACY_ANCHOR_TOLERANCE` 10 %; bei Johannes 0,5 % und 2,4 %). Ein Archiv mit 150/210 W
bekommt keine fremde Zahl, egal wie sein Schalter am Tag des Updates steht. Die Konstante
`LEGACY_STEERING_ANCHOR` liest nur diese eine Funktion; ein Wächter hält das fest.

**Johannes' Zahlen nachher = vorher:** SweetSpot 190 / VO2max 250, Pulsfenster 159–174 und
180–189, Bänder 186–194 und 235–265, `n_since` 0 — bitgleich (am Livebestand gerechnet).

**Warum der Prüfstand es nicht sah — und die neue Gattung:** **jede** Fixture trug Johannes'
Zahlen, also konnte kein Wächter unterscheiden, ob 190 aus dem Archiv oder aus dem Code kam.
Neu: `test_zweiter_athlet.py` — ein erfundener zweiter Athlet läuft durch die Handler, und in
keiner Payload darf eine Zahl oder ein Datum des ersten vorkommen. Am alten Quelltext 13 Lecks
in der ersten Payload; unter der Mutation „Startwert wieder aus dem Code" 11 rot.

**Lehre → Bauregel 10 (§13):** eine Zahl, die aus einem Bestand kommt, gehört ins Archiv dieses
Bestands, nicht in den Code.

---

**Achtundvierzigster Fall (0.67.0, 24.09.2026, das Umrechnungs-Paket — Entscheidung 23.09.
nach der Vorlage „Umrechnung alpha → Watt"): die Ermüdungskachel stand zur Hälfte auf einem
einzelnen Tempo-Block, und die Umrechnung hieß „gemessen".**

`bridges_alpha` mittelte zwei Quellen: den Stufentest (90,6 W je alpha) und eine „Blockleiter"
aus den zwei Blockfamilien-Sprossen, die alpha 1,0 am nächsten lagen — am Bestand Tempo (0,868,
**ein** Block einer Einheit vom 13.09.) und SweetSpot (0,680): 111,7. Ohne Regel: fällt Tempo,
rutscht die Leiter auf VO2max (0,402) und SweetSpot, 217,6, und Stunde 1 springt von 173,6 auf
191,4 W — ohne dass jemand etwas gemessen hätte. Ein Block um ±0,05 alpha verschoben bewegte
Stunde 1 um ±5 W. Dazu trug jede Zeile `measured: True`, und `state` sagte „aus gemessenem
alpha" — die Umrechnung selbst ist eine Setzung: Steigung einer Rampe (Minuten, steigende Last),
angewendet über Stunden; vier Arbeiten zur Dauerbelastung (van Rassel 2025, Gronwald 2024,
Ajayi 2025, Rogers 2021) zeigen alpha bei gleicher Last über die Zeit fallend, keine gibt eine
Umrechnung her, und die eigenen Grundlagenfahrten zeigen im Lastfenster keinen Zusammenhang.

**Gebaut in 0.67.0:** (1) `mid` ist der Stufentest allein; kein Rückfall auf die Leiter; fehlt
der Test, gibt es keine Wattzahl, und die Kachel zeigt den Verlauf in alpha mit dem Grund.
(2) Die Leiter wird weiter gerechnet und steht als **Gegenprobe** im Rechenweg — mit Zahl und
Satz; eine Störungsprobe sichert, dass sie keine Kachelzahl berührt. (3) `observed` statt
`measured`; `REVERSAL_WORDS["state"]` = „Setzung: Umrechnung aus deinem Stufentest";
Literatursatz und Rampensatz (Datum, Rate, Strecke) im Rechenweg; Setzungszeile der Kachel.
(4) Verfall: `RAMP_BRIDGE_MAX_AGE_MONTHS = 6`, Kalendermonate — bei Johannes am 16.03.2027
(15.03. steht die Kette, 16.03. nicht; ein frischer Test hebt es auf). (5) Tempo:
`blocks.HIDDEN_FAMILIES = ("tempo",)`, das Panel liest die Liste; die Familie bleibt, die
Einheit vom 13.09. bleibt, der Quellen-Reiter trägt den Satz „wird weiter gemessen, trägt
nichts".

**Johannes' Zahlen:** Kette 173,6 / 165,8 / 150,6 / 146,6 / 130,0 → **170,0 / 163,1 / 149,5 /
145,8 / 131,8** W (einzige Bewegung, gewollt: die Leiter fällt weg); „0,1 alpha ≈" 10,1 → 9,1 W;
Band ohne Brückenanteil, etwas enger. **Unverändert:** Kachelkopf 190 / 250, Pulsfenster
159–174 und 180–189, Steuerbänder, Anker-Puls, Kette B der Grundlage, alle FTP-Rückfälle.

**Prüfstand:** neun Zeilen rot vor dem Bau (Modul über Signatur, Panel über Wortlaut), nach dem
Bau grün; fünf Mutationen — Mittel aus beiden (7 rot), Rückfall auf die Leiter (4), Verfall
aus (2), Frist 7 Monate (3), Tempo-Kachel wieder da (2).

**Lehre:** eine Zahl, die aus zwei Quellen gemittelt wird, hat zwei Fehlerquellen und keine
Gegenprobe. Eine Quelle plus eine sichtbare Gegenprobe ist ehrlicher als ein Mittel.

---

**Neunundvierzigster Fall (0.67.1, 24.09.2026): der Kopf stand auf Stunde 1, die Zeigerleiste
auf Stunde 8 — und auf dem Handy dauerhaft.**

Beim Aufbau ruft das Panel `_fillReadout(name, null)` für jede Gruppe. Ohne Zeiger nahm die
Leiste den **letzten** Punkt („(zuletzt)") — die Regel einer Zeitreihe, bei der der jüngste Wert
der interessante ist. Die Ermüdungskachel hat aber eine **Stundenachse**: der letzte Punkt ist
Stunde 8, eine Fortschreibung („geschätzt, keine Messung · 0 Fahrten", 106 W / Studienform
93 W), während der Kopf Stunde 1 zeigt (169 W, ±62). Zwei Stunden auf einer Kachel; auf dem
Handy, wo niemand den Zeiger bewegt, dauerhaft. Nach der ersten Bewegung war alles richtig.

**Behoben in 0.67.1:** eine Gruppe kann ihren **Ruhepunkt** ansagen (`restIndex`); `_fillReadout`
nimmt ihn ohne Zeiger und schreibt „(Ausgangswert)" statt „(zuletzt)". Die Umkehrung sagt
Stunde 1 an (dieselbe wie der Kopf — die bestbelegte), die alte Ermüdungskachel die Rasterstelle
der Kopfstunde. Ohne Ansage bleibt die Zeitreihen-Regel. Eine Mechanik, zwei Ruhepunkte.

**Warum der Prüfstand es nicht sah — eigene Klasse „die Prüfung deckt den Aufbau nicht ab":**
die Hover-Prüfung aus 0.64.0 rief `_fillReadout(fatv2, 0)` und `(fatv2, 1)` und verlangte, dass
sich alle sechs Felder ändern — sie prüfte die **Bewegung**, nie den Zustand **davor**. Der
Aufbau ist ein eigener Fall mit eigenem Aufruf (`null`), und er ist der einzige, den ein
Touch-Gerät je zeigt. Neue Prüfung rot an 0.67.0 (zwei Zeilen: Stunde der Leiste, Zahlen der
Leiste), nach dem Bau grün, unter der Mutation „Ruhepunkt zurück auf zuletzt" wieder rot; die
Bewegungsprüfung bleibt als Gegenprobe grün.

**Lehre:** was ohne Eingabe auf dem Schirm steht, ist ein Zustand — und der erste, den der
meiste Bildschirm je zeigt. Jede Zeigerprüfung beginnt beim Aufbau.

---

**Fünfzigster Fall (0.67.2, 24.09.2026, Karte W4a.2 / F3.3 / F4a.4 — Sollzustand S4): eine
Einheitenkarte trug drei Fassungen derselben Wattzahl.**

Der Balken (`_woBar`) rechnete seinen Tooltip aus FTP × Katalogprozent, die Schrittliste
darunter las `blocks_w` (Kurve, Blöcke, Steuerung), und der Aufklapptext „wie es in Intervals
landet" kam bei gestreckten Wocheneinheiten aus `steps_text(stretched, ftp)` — wieder FTP.
Auf einer gestreckten Grundlagenkarte: 130 W in der Liste, 134 W im Text, Prozent im Balken
(der Wochenplan lieferte gar keine FTP, `plan.assessment.ftp` gibt es nicht — F4a.4).

**Behoben in 0.67.2:** `blocks_w` ist der eine Erzeuger (S4, aus „Geschrieben wird, was die
Karte zeigt"). Der Balken liest je Abschnitt die Watt aus `blocks_w`; Höhe und Farbe bleiben
am Katalogprozent (die Form der Einheit). `rate_sessions` überschreibt `text_w` nicht mehr —
`scaled()` hatte die gestreckte Vorlage schon richtig gerechnet. Keine Vorgabe bewegt sich; die
zwei falschen Fassungen fallen weg.

**Prüfstand:** rot vor dem Bau (Python 1, Panel 3 — dabei fiel auf, dass die Karten-Fixture
zwei `blocks_w`-Einträge gegen drei Abschnitte trug; berichtigt, weil `scaled()` beide gleich
lang baut); Mutationen „text_w wieder aus FTP" 1 rot, „Balken wieder aus FTP" 2 rot.

---

**Einundfünfzigster Fall (0.67.2, Karte F3.2 / W3.9): der Wochenplan baute seine Eingänge von
Hand — ohne Steuerung.** `websocket_goal` rief `rate_sessions` mit `ftp`, `curve`, `blocks`,
`ramp` aus eigener Zusammenstellung; `steering` fehlte. Bei Steuerung an zeigte der
Trainer-Reiter den Startwert (Johannes: 190/250), die Wocheneinheit derselben Familie den Median
der letzten Einheit. „EINE Stelle" (`_session_inputs`, Docstring seit 0.56.0) hatte eine zweite.
**Behoben:** `websocket_goal` liest `_session_inputs`; die Wochenkarte trägt dazu `watt_source`
und die Herkunftsfelder wie die Trainer-Karte. Rot am zweiten Athleten (Woche 1 SweetSpot:
Median 154 statt Startwert 150), Mutation `steering=None` 2 rot. Johannes' Zahlen: der Wochenplan
zeigt jetzt 190/250 statt des Medians — dieselbe Zahl wie die Karte, keine neue.

---

**Zweiundfünfzigster Fall (0.67.2, Karte F3.11): `_latest_ftp` verglich den Tag, nicht den
Zeitpunkt.** Bei zwei Aktivitäten desselben Tages gewann die Archivreihenfolge. Jetzt gilt die
spätere. Am Livebestand ohne Wirkung (die jüngste Aktivität mit FTP, 22.09. 15:51, steht allein
an ihrem Tag: 200 W nach beiden Regeln). Prüfstand: die Funktion erstmals **ungestubbt**
(Bauregel 8 — bis dahin ersetzte sie der Handlertest durch einen Wert), rot vorher 2, Mutation 2.

**Dreiundfünfzigster Fall (0.67.2, Sollzustand S2 nach Messung M-S2): vier Wege zur Tageslast.**
`analytics.daily_load` (ctlLoad, Lücken 0), `coach._acwr_local` (ctlLoad → `load`, keine
Lücken), `coach._day_load` (Aktivitäten → `load`), Plan und Kalender-Woche (Aktivitätssumme) —
im Kalender standen zwei davon in **einer** Zeile (F4a.9). Messung am Livebestand: ctlLoad und
Aktivitätssumme an 60 von 60 Tagen gleich, Abweichung 0 — die Wahl ist Bauhygiene.
**Behoben:** ein Erzeuger `analytics.load_by_day` mit der einen Regel „ctlLoad, sonst
Aktivitätssumme des Tages" (der Rückfall trägt den Fall aus 0.29.0, dessen Wächter beim Bau
hochkam: ein Konto ohne das Feld). Heute-Streifen, Signale-ACWR (jetzt `acwr_series`) und
Kalender-Woche lesen ihn; `_acwr_local` ist weg. Johannes' Zahlen: keine bewegt sich (Messung).
Prüfstand: Fixture mit absichtlich verschiedenen Reihen, rot vorher 2 + der alte Kalender-Wächter
(er erwartete die Aktivitätssumme — nachgezogen, dieselbe Klasse wie die No-op-Wächter bei F1.6);
Mutationen: Erzeuger auf Aktivitätssumme 6 rot, Kalender-Woche zurück 1, Heute zurück 1.

---

**Vierundfünfzigster Fall (0.67.3, F2.10 — Entscheidung 24.09.: Budget = Gesamtlast des Tages,
nicht Restlast).** `load_budget` zog `loads[-6:]` ab; die Reihe endet am letzten wellness-Tag,
und der ist am Livebestand heute (Last 0 bis zum Import). Also lagen heute und nur fünf Tage
davor in „den sechs Tagen davor", und nach dem Import schrumpfte das Budget um die Fahrt, die
es erlauben sollte. **Behoben:** die sechs Tage vor heute, chronisch über die 28 Tage vor heute,
die heutige Last als `used_today`; der Heute-Reiter sagt „Obergrenze X Last · davon Y gefahren".
Live heute (24.09., Ampel amber): Budget 0 → **10** (chronisch 25,6 → 27,8, weil die heutige 0
den Schnitt nicht mehr verdünnt; die sechs Tage davor 185 unverändert). Keine Karte ändert
sich (kleinste Last 18 > 10). Prüfstand: rot vorher (Signatur), Mutation 4 rot.

**Fünfundfünfzigster Fall (0.67.3, S5 / F4a.6 — Entscheidung 24.09.: eine Lastgrenze, die mit
Zustandsdeckel).** Der Heute-Reiter deckelte das Budget mit dem Zustand (`CAPACITY`, Setzung),
die Trainer-Karten lasen das Budget allein: an beanspruchten Tagen „passt ins Budget" neben
„Obergrenze 75". **Behoben:** `coach.load_ceiling(state, budget)` als der eine Erzeuger —
`CAPACITY` auf Modulebene, `today`, `websocket_workouts` und `websocket_goal` lesen ihn; die
Karten sagen „Obergrenze", nicht „Budget". Live heute: Zustand `ready`, kein Deckel → keine
Karte ändert sich; an einem `strained`-Tag würden Karten über 75 Last als zu groß markiert —
gewollt. Prüfstand: Z6 am zweiten Athleten rot vorher 3, Mutation „Budget allein" 2 rot.

---

**Sechsundfünfzigster Fall (0.67.4, F2.9 — Entscheidung 24.09.: Form absolut für beide
Reiter).** `summary()` stufte absolut (+6,8 → Grauzone), `readiness()` relativ (+21,5 % →
Übergang, amber) — dieselbe Tabelle, zwei Eingaben; bei CTL ~30 ist relativ dreimal so grob.
**Behoben:** `analytics.form_state(ctl, atl)` als der eine Erzeuger, absolut; der relative
Wert reist als `percent` mit. **Die eine Zahl, die sich bewegt (vorher genannt):** die Ampel
steht heute amber (Komponenten hrv/rhr/sleep/acwr grün, form amber, monotony/subjective
unbekannt); mit „absolut" ist form grün → Ampel **grün** → Ziel-ACWR 1,0 → 1,3 → Tagesbudget
**10 → 68** (auf 0.67.3-Zahlen: chronisch 27,8, sechs Tage davor 185). Damit passen z2_60 (45),
ramp_test (55) und recovery_40 (18) ins Budget; z2_150, SweetSpot, Schwelle, VO2max (78–115)
nicht. Prüfstand: rot vorher (Erzeuger fehlte), Mutation „wieder relativ" 3 rot.

**Siebenundfünfzigster Fall (0.67.4, S3 / F2.3 / W3.3 — Entscheidung 24.09.: Wahl 2 mit
Schranke).** Zwei Ketten aus denselben Stunden: die Kachel las `_plan_chain` bei der geplanten
Dauer, die Einheit baute in `curve_watts` eine eigene (Abbruch < 6 Paare, Stundenmitte des
Hauptteils, Studienform darüber hinaus); der Nachweis sagte „für eine Fahrt von 2 h" an einer
3,5-h-Einheit. **Behoben:** `curve_watts` liest `curve["plan"]` (Kette A) bei der geplanten
Dauer — ein Erzeuger; **Rundung** benannt in `planned_hour`: nur volle Stunden, mindestens
eine (2,5 h liest bei 2 h); **Schranke** `CURVE_HOUR_MIN_RIDES = 3`: unter drei Fahrten gilt die
letzte gut belegte Stunde davor, auch jenseits des belegten Bereichs — keine Studienform mehr in
der Einheit. Die 0,90 (`CURVE_TARGET_SHARE`) bleibt: sie ist mit der Pulsseite begründet, nicht
mit der Ablesestelle. **Live je Einheit (Schwelle ×0,90):** z2_60 133 → 133 · z2_90 133 → 133 ·
z2_150 136 → 136 · z2_210_late **136 → 126** (Stunde 3 statt 2) · längste Fahrt 5,5 h **131 → 126**
(Stunde 3 statt Studienform) · Woche z2_150 gestreckt 6,0 h 125 → 126. Prüfstand: rot vorher
(`planned_hour` fehlte, Livezahlen als Sollwerte), drei Mutationen (Stundenmitte 4, Schranke weg
5, Rundung halb-auf 4) rot; die alten Kurven-Wächter, die Studienform und Paar-Schranke
festschrieben, nachgezogen; Fixtures tragen jetzt `plan` (Kette A), wie `fatigue.curve` es liefert.

**Achtundfünfzigster Fall (0.68.0, GA-Umkehrung — Auftrag 25.09., der offene Widerspruch
„zwei Zahlen für dieselbe Sache" aus der Übergabe vom 24.09.).** Die Grundlageneinheit stand auf
der p075-Kette × 0,90 (z2_60 133 W), die Kachel zeigte bei Rechenschalter an die Umkehrung
(170 W) — zwei Antworten auf zwei Fragen, ohne Verweis. **Entscheidung Johannes:** die Einheit
beantwortet die Frage der Kachel. **Gebaut:** `fatigue_v2.ga_targets` ist der eine Erzeuger —
je belegter Stunde Ziel = Last + (α − Ziel-alpha) × Steigung und Grenze = Last + (α − 1,0) ×
Steigung, Steigung nur aus dem Stufentest (0.67.0); Ziel-alpha ist eine **Integrationsoption**
(`ga_alpha_target`, leer erlaubt), Grenz-alpha Option mit Vorgabe 1,0. `workouts.ga_at` liest
an derselben Stelle wie 0.67.4 (volle Stunden, ≥ 3 Fahrten, sonst die letzte gut belegte); der
Hauptteil trägt das Ziel, ohne Ziel die Grenze; ohne gültigen Stufentest die FTP mit Grund
(`ga_missing`). `curve_watts` ist weg, die alte Kette (p075) rechnet nicht mehr in die Einheit; die 0,90
(`CURVE_TARGET_SHARE`) steht nur noch am Start des Stufentest-Protokolls als Rückfall OHNE
Umkehrung (HRVT1 × 0,90) und an der Stufe `ramp_hrvt1`, die keine Familienkette mehr nennt. **Neu:** ab 3 h geplanter Dauer trägt die Einheit
`unverified` („ab 3 h ungeprüft — Abnahmefahrt offen"; `GA_UNVERIFIED_FROM_HOUR = 3`), auch
wenn sie mangels Belegung an Stunde 2 abliest — gefahren werden drei Stunden. Die Karte sagt
„fahr ~X W, nicht über Y W", Stunde und Fahrten, „Setzung"; die Schrittliste markiert den
Abschnitt „Umkehrung". **Live je Einheit (Ziel 1,3, Grenze 1,0):** z2_60 133 → **143/170** ·
z2_90 133 → **143/170** · z2_150 136 → **136/163** · z2_210_late 126 → **122/150** · 5,5 h und
6 h 126 → **119/146**; ohne eingetragenes Ziel steht die Grenze (170/163/150/146). Blockvorgaben
190/250 und Pulsfenster unbewegt. **Zweiter Athlet (neue Grundregel je Schritt):** Athlet B ohne
Stufentest, HRV, Etiketten, Marken und Ziel läuft durch `websocket_workouts` ohne Absturz, die
Grundlage fällt BENANNT auf seine FTP; mit eigenem Stufentest (80 W/alpha) und ohne Ziel trägt
sie nur seine Grenze (117 W). Verbotsliste um 90,6 · 101,2 · 160,8 · 1,3 erweitert; Grep-Beleg
am Produktionscode: keine der Zahlen außerhalb von Kommentaren (die 1,3 in `analytics.ACWR_HIGH`
und den ACWR-Bändern des Panels ist die Literaturgrenze 0,8–1,3, kein Ziel-alpha; 190/250 nur in
`LEGACY_STEERING_ANCHOR`, §7 Fall 49). Prüfstand: acht Panel-Zeilen und vier Rechnerzeilen rot
vor dem Bau; sieben Mutationen rot (0,90 zurück 8+1+1 · Ziel 1,3 aus dem Code statt Option 5 ·
Grenze statt Ziel 6+1 · `unverified` nie gesetzt 3 · Karte ohne Abnahmefahrt-Hinweis 1 ·
90,6 als Rückfall ohne Stufentest 1+7 · Karte erfindet ein Ziel mit 1,3 2).

**Neunundfünfzigster Fall (L1, 0.69.0 — Auftrag 25.09., aus der Zeichnung des Sollzustands).**
Zwei Regeln entschieden über die ART der Einheit: der Zustand (HRV, `fit_for`) und das
Lastbudget (ACWR, `stage`: „Das Lastbudget verbietet es heute"). Am Livebestand der letzten
30 Tage sperrte das Budget an 16 Tagen die Grundlage bei tragendem Zustand (09.02, 04, 18, 19,
22–25.09 bereit; 03, 05, 21.09 beansprucht; 08, 09, 15.09 erholend; 10, 17.09 Rückkehr) und
schob im Panel die Empfehlung auf die Art, die ins Budget passte (Wächter 12: „die erste
ok-Karte IM BUDGET führt" — SweetSpot statt Grundlage). **Entscheidung Johannes:** der Zustand
entscheidet die Art, die Last ist ein beschriftetes Geländer an der Menge. **Gebaut:** `stage()`
nimmt die Stufe aus `fit`; über der Obergrenze `over_ceiling` und ein Satz („Art bleibt, Menge
kürzen"), rot nur noch aus dem Zustand. `guard()` nennt Last, Obergrenze und — bei elastischen
Einheiten — bis zu welcher Dauer die Fahrt darunter passt (Last ∝ Dauer, `session_load`,
Viertelstunden abgerundet); eine feste Einheit sagt „nicht kürzbar". Die Reiz-Stufe (bereit,
über Budget, Erholung im Rücken) bleibt als Ja mit Beleg. **Athlet ohne HRV** (Zustand
`unknown`): dort entscheidet die Last (`by_load`) — rot am Budget, mit dem Satz „Ohne Zustand
(keine HRV-Basislinie) entscheidet die Last" (Z8). Panel: Geländer auf Karte und Leitempfehlung,
die erste grüne Karte führt. **Setzung der Tabelle:** je Tag mit Budget bei Ampelziel 1,0
gerechnet (die Ampel je Vortag ist aus der Payload nicht rekonstruierbar), Zustandsdeckel aus
`CAPACITY`. Prüfstand: rot vorher (Wahrheitstabelle, `by_load`, Geländer-Felder, drei
Panel-Zeilen, Wächter 12), sechs Mutationen rot (`by_load` ignoriert 5+1 · alte Regel zurück 14+2
· passende Dauer aus dem Katalog 3 · Wochenkarte ohne Geländer 5 · Karte ohne Geländer-Zeile 1+1+1
· Empfehlung weicht ins Budget aus 2+4).

**Sechzigster Fall (S1, 0.69.0 — Auftrag 25.09., Karte 4b F4b.3 / Karte 7 S1).** Drei
HRV-Basislinien im Paket, alle „±0,5 SD" beschriftet: der Trainer (`coach.state`) 60 Nächte VOR
heute, gewichtet; die Ampel (`analytics.hrv_status`) Mittel und Streuung der letzten 60
**Rollwerte einschließlich heute**, ungewichtet; die Signale (`coach.signals`) ungewichtet; dazu
die Anzeige-Bänder des Heute-Reiters (`_signal_bands`) mit der heutigen Nacht im eigenen Band.
Die Belastung konnte „im Normalbereich" sagen, während der Trainer „beansprucht" zeigte.
**Entscheidung Johannes (Wahl 1):** alle gewichtet, ein Erzeuger. **Gebaut:** `baseline.py` —
`norm_band`, `z_at`, `z_series`, `band_before`, `fallback_note`, `weights_for`; `coach.py`
behält die Namen als Durchreiche (Wächter 29 nachgezogen), `analytics.hrv_status` liest
`band_before` (analytics liest `day_context` weiter nicht direkt, nur über baseline), die
Ampel-Komponente trägt `weighted`/`labeled` und nennt das Band, `readiness.context_note` kommt
aus dem Band („gewichtet gerechnet — N etikettierte Tage … dieselbe Basislinie wie beim
Trainer"), der Handler hängt nichts mehr an. Panel: derselbe Satz in Belastung und Signale.
**Folge an Johannes' Zahlen (30 Tage, lokal nachgerechnet mit 82 Tagen Bestand):** die
Gewichtung selbst bewegt fast nichts (5 etikettierte Nächte von 60, z-Werte ±0,05); was sich
bewegt, ist die **SWC**: 0,5 × Streuung der **Nächte** (Plews) statt 0,5 × Streuung der
7-Tage-Mittel — das Band wird breiter, die Ampel-Komponente HRV an 7 Tagen milder (31.08.,
05.09. amber → grün; 06.–08., 10.09. rot → amber; 11.09. rot → grün). **Setzung, benannt:**
die SWC-Basis folgt aus „ein Band"; wer die alte, engere SWC will, braucht wieder eine zweite
Rechnung. Ebene 3 bleibt für die Last: Budget, ACWR, Monotonie lesen keine Gewichte (Wächter
nachgezogen: `hrv_status`/`readiness` aus der kontextfreien Liste, Gegenprobe „liest die
Etiketten"). Prüfstand: rot vorher (Import `baseline`, Signale-z, Panel-Sätze, Ebene-3-Liste),
fünf Mutationen rot (Ampel ungewichtet 8 · Fenster einschließlich Tag 1 · Signale ungewichtet 3
· SWC aus Rollwerten 1 · Gewichtung tot 6+7).

> **Nachtrag 0.72.1:** die Vorlage „gewichtete Nächte → nicht bewertbar“ ist gebaut (Fall 67).

**Einundsechzigster Fall (L2, 0.69.0 — Auftrag 25.09., Station ④ der Zeichnung).** Die Station
„Bewertung des Reizes" fehlte als Erzeuger: `night_after` verglich die Nacht mit der üblichen
Antwort auf ähnliche Einheiten (Referenz aus ≥ 5 Vergleichsnächten), aber nirgends stand, ob der
Reiz verdaut ist. **Entscheidung Johannes:** nur als Anzeige, als Setzung beschriftet, kein
Eingang in den Trainer — verdaut ≥ −0,5 SD · gekostet < −0,5 · zu viel < −1,0 ODER zweite Nacht
< −0,5. **Gebaut:** `coach.night_verdict(z_first, z_second)` (Konstanten `NIGHT_DIGESTED_Z`,
`NIGHT_TOO_MUCH_Z`, `NIGHT_SECOND_Z`, Wörter in `NIGHT_VERDICT_WORDS`), gerechnet aus dem HRV-z
der Nacht danach und der zweiten Nacht gegen das eine Band (S1); `night_after` trägt `verdict`
(Wort, beide z, Regel, `setting: True`, „zweite Nacht liegt noch nicht vor"). Panel: eigener Kasten
(`nverdict`) im Nacht-Block der Aktivität und im Heute-Reiter — bewusst getrennt vom
Referenz-Urteil „wie sonst nach solchen Einheiten", das eine andere Frage beantwortet. **Am
Livebestand (lokal, 82 Tage):** 27.08. (62) zu viel · 30.08. (93) gekostet · 01.09. (79) zu viel —
erste Nacht −0,23, zweite −0,63: die ODER-Regel greift · 02.09. (40) gekostet · 04.09. (129) zu
viel · 09.–16.09. verdaut · 20.09. (90) zu viel (−1,01) · 22.09. (95) verdaut. Prüfstand: rot
vorher (Konstanten, Feld, Panel-Kasten), sechs Mutationen rot (zweite Nacht ignoriert 1 ·
verdaut-Schwelle 2 · gekostet→zu viel 1 · zweite Nacht nie gelesen 3 · Karte ohne Bewertung 6 ·
`load_ceiling` liest die Bewertung 1).

**Zweiundsechzigster Fall (0.69.1 — drei Nachbesserungen aus dem Live-Blick, 25.09.).** *(1) L2
geschärft.* Die ODER-Regel von 0.69.0 ließ die zweite Nacht ALLEIN auf „zu viel" springen — am
Livebestand die Nacht nach dem VO2max vom 01.09. (erste −0,23, zweite −0,63). Das war zu hart:
eine Nacht im Band und eine darunter ist „gekostet, verzögert". Neue Regel in `coach.night_verdict`:
zu viel = erste < −1,0 ODER (erste < −0,5 UND zweite < −0,5); gekostet = erste < −0,5, oder
verzögert (Feld `delayed`) bei erster im Band und zweiter < −0,5; verdaut sonst. Wahrheitstafel
mit den Rändern (genau −0,5 ist verdaut, genau −1,0 noch nicht zu viel) und der Livewert 01.09. als
Sollwert im Prüfstand; zwei Mutationen über Dateikopie (alte ODER-Regel zurück · verzögerter Zweig
weg) je 6 rot. *(2) „passt heute" log über der Obergrenze.* Die grüne Karte trug das Geländer als
Zeile, das Etikett sagte trotzdem „passt heute". Jetzt setzt `workouts.stage` über der Obergrenze
das Wort aus einer Stelle (`GUARD_WORDS["over_word"]`, „Art bleibt, Menge kürzen"), grün wie gelb;
das Panel hängt kein „heute" mehr dahinter, und die Leitempfehlung nennt die passende Dauer aus
`guard()` („heute ~1,5 h (Vorlage 90 min)"). Gegenprobe: unter der Obergrenze bleibt „passt heute".
Rot vorher: 6 (Backend, KeyError am Register) + 5 (Panel). *(3) Listenkopf (KARTE_4a W4a.1).*
„Watt aus deiner FTP" stand seit 0.68.0 falsch über der Einheitenliste — jetzt: Grundlage aus der
Umkehrung, VO2max/SweetSpot aus dem Steuerwert, sonst FTP; rot vorher 2. Keine Rechnung an einer
Vorgabe berührt: Blockvorgaben, Pulsfenster, Ziel/Grenze der Grundlage unbewegt.
**Dreiundsechzigster Fall (0.69.2 — zwei Fehler, die falsch fahren ließen; Live-Blick 25.09. nach
einer VO2max-Einheit).** *(F1) Die Karten nahmen scheinbar die FTP statt der Vorgabe.* Die
SweetSpot-Karte trug 190 W — das WAR die Vorgabe (Steuerung, 6 Einheiten) — aber mit dem Etikett
„Rückfall auf die FTP — nicht gemessen“ und 0 gewerteten Einheiten. Ursache `workouts.explain()`
(Z. 1932 ff., 0.69.1): der Nachweis kannte die Quelle `steering` nicht und fiel in den FTP-Zweig; die
Zahl war richtig, die Herkunft gelogen. Jetzt ein eigener Zweig (Startwert + Schritte × 5 W,
Toleranz, Pulsfenster, gewertete Einheiten aus derselben Reihe wie die Kachel). Die VO2max-Karte
(30/30) stand wirklich auf der FTP (210 W) — das ist Entscheidung 0.61.0 („Etiketten ehrlich“): die
Vorgabe gilt für **Arbeitsblöcke** (`_is_work_block`: „Block …“ oder „1“–„4“), ein 30/30- oder
30/15-**Satz** bekommt sie nicht. Sie gilt also je Blockform, nicht je gemessener Variante (4×4,
5×4, 4×8 bekommen sie; 4×8 als Übertragung benannt). Die Karte sagte dazu „noch zu wenige gemessene
Einheiten“ — falsch, es gab 7. Jetzt: „Rückfall auf die FTP — deine Vorgabe (250 W aus 7 Einheiten)
gilt für Arbeitsblöcke; diese Form bekommt sie nicht“, im Nachweis wie im Herkunftsabsatz. Rot
vorher 10 (Backend) + 4 (Panel), Mutationen über Dateikopie: `steering`-Zweig weg 6 rot,
ehrlicher FTP-Zweig weg 3 rot. Gegenprobe: Familie ohne Messung fällt weiter auf die FTP, „nicht
gemessen“. Offen als Vorlage V1: Block „5“ von `vo2_5x4` fällt auf die FTP, weil die Etikettregel bei
„4“ endet — seit 0.61.0 so benannt („Block 5 als Rückfall“), vermutlich Versehen, nicht angefasst.
*(F2) Nach dem Training stand die Obergrenze auf allen Karten bei 0.* Der Reiter sagte „gelten für
morgen“, `websocket_workouts` (Z. 1119) und `websocket_goal` (Z. 1433) lasen aber
`load_ceiling(state, readiness.budget)` — das heutige Budget, nach der Einheit aufgebraucht, bis
hinunter zur Regeneration „nicht kürzbar“. Jetzt EINE Stelle, `coach.session_ceiling`: ist heute
trainiert (`_trained_today`), rechnet sie `analytics.load_budget(…, today=morgen)` — heute zählt zu
den sechs Tagen davor (F2.10), Verbrauch 0 —, sonst das heutige Budget bitgenau wie vorher; der
Zustand bleibt der von heute (für morgen gibt es keinen). AST-Wächter: beide Handler lesen
`session_ceiling`, keiner mehr `load_ceiling` direkt. Dazu `guard()`: „nicht kürzbar“ nur noch für
FESTE Einheiten (Arbeitsblöcke oder Stufentest); eine elastische Grundlage oder die Regeneration
gegen Obergrenze 0 sagt „auch gekürzt passt sie nicht unter die Grenze“. Rot vorher 1 (Attribut)
+ 4 (guard), Mutationen: Morgen-Zweig weg 5 rot, Handler zurück auf `load_ceiling` 1 rot,
`fixed = not elastic` 1 rot. Gegenproben: vor dem Training identisch zu `load_ceiling`; unter 28
Tagen kein Budget; feste Einheit sagt weiter „nicht kürzbar“; Grenze 50 nennt die Dauer 0,8 h.
*(B, nur Befund)* die VO2max-Steuerung ruht auf 4 Blöcken 0,85/0,63/0,35/0,30 bei einer 3×4-Einheit:
die Blöcke sind die markierten und gemessenen Abschnitte der Familie (Blockschalter auf „meine
Markierungen“, `blocks._marked_sessions`) bzw. die WORK-Blöcke der Erkennung; der Kachel-Median
0,490 nimmt alle vier, die Steuerung zählt ab Block 2 (0,347) — Block 1 mit 0,85 ist kein
eingeschwungener VO2max-Block und gehört vermutlich zu einem markierten Einroll- oder Anlauf-Lap.
Live nachzusehen: Aktivitäten → die 3×4-Einheit → markierte Abschnitte (welche vier Laps tragen
die VO2max-Marke) und im Quellen-Reiter die Stellung des Blockschalters. Keine Vorgabe bewegt
außer der reparierten Quelle (Grenze von morgen).

**Vierundsechzigster Fall (0.70.0 — der Trainer-Reiter nach der Skizze, Fassung 2; Live-Blick
25.09.).** Kein Rechenfehler als Anlass, sondern die Seite: zwischen Zustand und Empfehlung standen
Legenden, Quellen, Entkopplung und eine erledigte Frage. *A (Reihenfolge):* Kopf als eine Zeile ·
Zustand als eine Zeile (Wort aus `state.label`, Satz aus dem neuen `state.short`, Balken und
Begründungen zu) · eine Empfehlung mit der Dauer aus `guard()` und der Unterzeile „für morgen“ ·
drei Familien (Grundlage · SweetSpot & Schwelle · VO2max) je ein Aufklapper mit Etikett, Watt +
Kurzherkunft, empfohlener Variante, Nutzen · Stufentest als Zeile · nur diese Woche offen, W2 ff.
und die Erklärtexte im Rechenweg · Hintergrund (Anker, Kurve, Leistung je Block) ganz unten, zu.
Die Anordnung ist eine Panel-Konstante (`TRAINER_FAMILIES`), keine Rechnung; Watt der Familienzeile
aus `explain.headline` (derselbe Erzeuger wie „Wie diese Zahl entsteht“), Kopfzahl der Kachel aus
`_famHead` (eine Stelle für Kachel und Zeile). Offene Aufklapper überleben das Neuzeichnen
(`data-keep`, toggle-Hörer). *B (Umzüge):* Entkopplung → Fitness (dieselbe Payload
`coach.durability`), Stufentest-Arbeiten, „Worauf diese Empfehlung beruht“, Stufen-Legende mit
Erholungsregel und „Wer hier entscheidet“ → Quellen (lädt dafür `goal`); Kalendersatz → Tooltip an
jedem Kalenderknopf; ein Satz je Stufe als Tooltip am Etikett (`stage.detail`); „Die offene Frage:
56 Watt“ (`rRampGap`, `_auswahl`) ersatzlos gestrichen. *C:* (C1) die Stufentest-Karte fiel seit
0.69.2 in den Steuerungszweig von `_sourceText` („– W: Startwert – W vom –“) — sie trägt
`watt_source = steering` (Rampenende), aber kein `steering_source`; jetzt zuerst der eigene Satz aus
`ramp_protocol` (Start/Ende mit Herkunft, Reserve als Setzung). (C2) „Die Steuerung ruht auf 4
Blöcken … Median 0,490“ bei eingeschalteter Vorgabe falsch — gezählt wird ab Block 2; jetzt die
gezählten Blöcke, der Median aus der Zeile der Steuerung (0,347), Block 1 „markiert, nicht gezählt
(Anlauf, Rogers)“; ohne Vorgabe unverändert. (C3) „Diese Messung steuert noch keine Vorgabe“ →
„steuert die Grundlage“ (Umrechnung der Umkehrung). (C4) Stufentexte auf L1 (`STAGES`), „höchstens
0 harte Tage“ → „kein harter Tag“ (`coach._hard_days_phrase`). (C5) nur Befund + Vorlage, §10.
(C6) „erst 1 von 3 Einheiten seit dem Startwert“ → „1 Einheit seit dem Startwert, eine daneben —
bewegen kann sich die Vorgabe frühestens ab 3 Einheiten“. (C7, V1) eine Etikettregel
`_is_work_block` für alle drei Zweige, jede Blockzahl: Block 5 einer 5×4 bekommt die Vorgabe
(**die einzige bewegte Zahl: 224 → 250 W an diesem einen Block**). (C8, Entscheidung Johannes) eine
Erholungseinheit ist nie über der Grenze: `GUARD_EXEMPT_FAMILIES = ("recovery",)`, gefragt von
`guard()`, `suggest()`, `rate_sessions()`. Keine andere Rechnung, keine Vorgabe bewegt. Rot vorher
13 + 4 + 2 (Backend) und 62 (Panel, erste Fassung; C2, Aufklapper und Kalender-Tooltip danach
geschärft, je rot vor dem Bau), Mutationen über Dateikopie 36, alle rot — zwei erst nach
Schärfung: C6 „daneben“ fand die Mutation nicht, weil „keine daneben“ das Teilwort „eine daneben“
enthält (geschärft mit Trefferzusicherung und Gegenprobe im anderen Zweig — die Bauregel
„Trefferzusicherung nur auf dem Zweig der Fixture“ hat wieder getroffen); der Kalender-Tooltip wurde
nur irgendwo statt an jedem Knopf geprüft. Befund nebenbei, nicht repariert: der Grundlagen-Herkunftsabsatz
(`_gaText`) steht ohne umschließendes `<span>` in einem Flex-Absatz und zerfällt optisch in Spalten.

**Fünfundsechzigster Fall (0.71.0 — Skizze 0.71.0, Abschnitte 1–3; Abschnitt 4 nicht angefasst).**
*(1) Die drei Zustandswerte immer sichtbar.* Bis 0.70.0 standen HRV 7 T, HRV 3 T und Ruhepuls 3 T
nur im Aufklapper „Warum“. Jetzt drei Mini-Streifen unter der Kopfzeile (unter 600 px
untereinander). Kleiner und großer Streifen kommen aus EINER Funktion `_zRow(label, z, col, size)`:
Achse −3…+3 SD, Band ±0,5 SD und Nulllinie aus denselben CSS-Klassen, dieselben Farben je
Signal, Werte außerhalb am Rand mit echter Zahl, fehlender Wert „keine Daten“ ohne Punkt. Keine
neue Schwelle, keine neue Farbe; die Positionsformel steht einmal im Quelltext. Der Zustandssatz
stand im Aufklapper doppelt (`state.detail` und erster Grund aus `coach.reasons`) — die Zeile
entfällt, wenn ein Grund denselben Text trägt. *(2) Köpfe im zugeklappten Hintergrund:*
„Grundlage 169 W (1 h, ±29 W · Umkehrung) · SweetSpot 190 W (186–194 · Vorgabe) · VO2max 250 W
(235–265 · Vorgabe)“ (Fixture-Zahlen). Die Köpfe lesen `_fatHead(f)` (Ruhezeile der
Ermüdungskachel, v2: Stunde 1 mit Spanne, alte Kurve: erste Stunde) und `_famHeadFull(b, key)`
(Kopfzahl, Band nur bei Vorgabe, Herkunft) — dieselben Helfer, aus denen die Kacheln ihre
Ruhezahl lesen (`rFatigueV2` `at(null)`, `rFatigue` `erster`, `_famValue`). Ohne Kachel oder bei
ausgeblendeter Familie: „noch keine Kachel“. *(3)* `_gaText` hat jetzt ein umschließendes `<span>`
im Flex-Absatz. Rot vorher 36, danach nachgeschärft (alte Kurve, zwei Gegenproben ohne Kachel);
Mutationen über Dateikopie 16, alle rot — eine erst nach Schärfung: die Gegenprobe ohne Kachel
löschte Familie und Vergleichszahl zugleich und traf den Familien-Check nie allein. Keine Rechnung,
keine Vorgabe bewegt.

**Sechsundsechzigster Fall (0.72.0 — Skizze 0.72.0, Abschnitte 1–4; Entscheidungen Johannes
26.09.: Ruhepuls nicht spiegeln, je Familie mindestens fünf belegte Einheiten, C5 Wahl 1).**
*(1)* `suggest()` bewertet jede Variante einer Familie an EINER Stelle (`judge`: `scaled` →
`fit_for` → `stage` → `guard`) und liefert sie als `variants`; der Handler hängt jeder ihren
Nachweis an. Das Panel zeigt sie als gekürzte Karten, die empfohlene oben mit Marke; die Zeile
„Weitere Varianten“ ist ersetzt, die zugeklappte Familienzeile unverändert. `alternatives` reist
weiter mit. *(2)* Drei Einheiten: `threshold_4x16` (Familie SweetSpot → Vorgabe, sonst FTP; Seiler
2013, Sylta 2016), `threshold_5x6` und `threshold_3x15` (Familie Schwelle → FTP; Stöggl & Sperlich
2014). SOURCE_CHAIN unverändert; Blockhöhe und Intensität wie die Geschwister, Last aus den
Blöcken gerechnet (Dauer × IF² × 100, Setzung); die Übertragung HF/Laktat → Watt als Setzung
beschriftet; der Neal-Zusatz (`NEAL_NOTE`) an allen Schwellen-Formen. SweetSpot & Schwelle hat
jetzt sieben Einheiten. *(3)* Jede Karte trägt ihre Kennung (`EVIDENCE_KIND`, genau nach der
Tabelle des Vorarbeiters; alter Belegtext bleibt dahinter). Nicht in der Tabelle, aus dem eigenen
Belegtext gelesen und zur Prüfung vorgelegt: Stufentest „Studie“, Wiedereinstieg „Konvention“.
*(4)* Der große Tag ist durch `coach._progression` gedeckelt: `_state_for_plan` liest
`coach.durability().progression`, `plan()` nimmt `next_minutes` als Grenze des großen Tages (nur
dort), `big_day_cap` trägt Grenze, Bezug, Faktor, `applied` für den Rechenweg („Großer Tag X h —
gedeckelt durch die Progression aus deiner längsten Fahrt Y“). **Bewegt die Wochenstunden der
Woche des großen Tages (freigegeben).** plan.py kennt den Faktor nicht. Rot vorher 46 + 5 (Backend)
und 11 (Panel); Mutationen über Dateikopie 19, alle rot — zwei erst nach Schärfung (die
empfohlene Variante war in der Fixture schon die erste; die Grenze lag über dem Routinetag).

**Siebenundsechzigster Fall (0.72.1 — die Nachtbewertung liest das Tagesetikett; Skizze 0.72.1).**
Befund: `night_after`/`night_verdict` lasen das Etikett der bewerteten Nacht nicht — eine Nacht
mit Alkohol, Nachtschicht oder krank wurde der Einheit zugeschrieben; auch die Vergleichsgruppe
nahm etikettierte Folgenächte früherer Einheiten. Die Vorlage L2 (Zeichnung 24.09.: „gewichtete
Nächte → nicht bewertbar“) war nicht gebaut — **jetzt erledigt.** *(1)* Trägt die erste ODER die
zweite Nacht nach der Einheit ein Etikett mit Gewicht < 1 (`day_context.weight_for`, der eine
Erzeuger; Tag D = Nacht, die am Morgen D endet), gibt es kein Urteil: L2 `key =
"nicht_bewertbar"`, Vergleich `state = "unrated"`, beide mit dem Satz „Nacht zum TT.MM. mit
Etikett X, Gewicht Y — sie misst nicht nur die Einheit“ (`coach._night_label`). Rohwerte
(z beider Nächte, Nachtzeilen, Referenz) bleiben. Ein Etikett am Tag der Einheit selbst (die
Nacht davor) ändert das Urteil nicht. *(2)* Folgenächte früherer Einheiten mit Gewicht < 1 zählen
nicht in die Vergleichsgruppe; unter fünf bleibt es bei „kein Vergleich möglich“. *(3)* Etikett
`cannabis` (Gewicht 0,5 wie Alkohol, Setzung), Lesetext und Beleg am Etikett (Gonzalez et al.
2026, J Sleep Res, PMID 41692699, Pilotstudie, 18 Erwachsene), dazu im Quellenblock des Etiketts;
die alten Etiketten per Prüfsumme unverändert. Panel: „nicht bewertbar“ bekommt einen eigenen,
neutralen Ton mit Info-Zeichen statt des grünen Häkchens; Cannabis einen achten Kategorienton
(`C.sand`), weil die sieben vorhandenen je ein Etikett tragen. Nur Anzeige — keine Vorgabe liest die
Nachtbewertung. Rot vorher 15 (Backend) + 3 (Panel); Mutationen über Dateikopie 14, alle rot.

**Achtundsechzigster Fall (0.72.2 — das Stufenwort steht immer da, die Menge ist ein eigenes Zeichen; Skizze 0.72.2).**
Befund: `workouts.stage()` ersetzte über der Obergrenze (grün/gelb) das Wort durch
`GUARD_WORDS["over_word"]` („Art bleibt, Menge kürzen“) und den Satz durch `green_over`/`yellow_over`
— bei Obergrenze 0 trug jede Karte dieses Wort, der Zustand verschwand bis auf die Farbe. Eingeführt
0.69.1; Fehler: zwei Aussagen in einem Etikett. *(1)* STAGES trägt die Worte der Entscheidung
Johannes 26.09.: „passt {tag}“, „geht, kostet mehr“, „gewollter Überreiz“, „{tag} nicht“; das Panel
setzt den Tag an EINER Stelle ein (`_stageWord`: „morgen“, wenn heute schon trainiert, sonst
„heute“); das Wort und `detail` werden nie ersetzt. *(2)* Über der Obergrenze bei grün/gelb setzt
`stage()` `quantity = {label: „Menge über Wochenlast“, text: green_over/yellow_over}`; das Panel zeigt
es als kleines Zeichen neben der guard()-Zeile (`_qtyMark`). Kein Zeichen bei Reiz, rot, `by_load`,
Regeneration (guard_exempt), ohne Budget. *(3)* Karte, Familienzeile, Empfehlung oben (neu mit
Etikett), Wochenplan-Chips (vorher das Register-Wort, dazu „(über der Obergrenze von X)“ — entfällt)
und Legende im Reiter Quellen lesen dieselben Worte. *(4)* `over_word` entfernt; `green_over`/
`yellow_over` nur noch als Satz am Zeichen. Rechnung unverändert (Stufe, Budget, guard, Watt/Puls).
Rot vorher: Backend Absturz am fehlenden `quantity`, Panel 24; Mutationen über Dateikopie 28, alle rot.

**Neunundsechzigster Fall (0.72.3 — der Beschriftungsdialog sagt, welche Nacht gemeint ist; Auftrag 26.09.).**
Befund: ein Etikett am Tag D wirkt auf die Wellness-Zeile D (`day_context.weight_for(data, D)`),
deren Garmin-HRV die Nacht ist, die am Morgen D endet. Der Dialog sagte nur „Tag beschriften —
26.09.2026“; dass der Abend des 25.09. gemeint ist, stand nirgends. Jetzt: „Nacht zum {dMed(D)}
beschriften — also der Abend des {dMed(isoMinus(D, 1))}“ in `_ctxPopover()`, aria-label „Nacht
beschriften“. Vortag aus `isoMinus` (UTC), kein eigener Date-Weg. Sonst nichts geändert (Etiketten,
Gewichte, Texte aus day_context.py, andere Reiter). Rot vorher 6 (26.09., Monats-, Jahreswechsel,
Zeitumstellung 26.10. → 25.10., „Tag beschriften“ weg, aria-label); grün auch unter TZ
Europe/Berlin, Pacific/Kiritimati, America/Los_Angeles; Mutationen über Dateikopie 6, alle rot
(darunter `isoMinus(D,1)` → D und ein lokaler Date-Weg, der an der Zeitumstellung fällt).

**Siebzigster Fall (0.73.0 — der Heute-Kopf: Körper oben, Wochenlast je Familie darunter, Fahrten rechts; Skizze 0.73.0 §4–§12).**
Befund (Skizze §1): oben „Alles möglich“ (Zustand ready, `CAPACITY` ohne Deckel), darunter „Obergrenze 0 Last“
mit „darüber wird es ein Reiz, den du heute nicht verdaust“ — die 0 kam aus der Wochenlast
(`load_budget`), der Satz gehörte zu einer Zustandsgrenze. *(1)* `workouts.FAMILY_GROUP`/`KEY_GROUP`/
`GROUP_LABEL`/`GROUP_ORDER` — eine Tabelle Katalogschlüssel → Gruppe, gegen `TRAINER_FAMILIES` am
Quelltext geprüft. *(2)* `analytics.activity_family(data, id, events)`: Marken → Plan (paired_event_id →
Event; external_id mit Präfix `ha-intervals-icu:` gelesen, Altbestand = Hinweiszeile aus `to_event` +
Katalogtitel) → andere Sportart → None; Pendelfahrt nie über den Plan; mehrere Gruppen: die härteste
(Setzung E3). Kein Namensvergleich, keine Zonen, kein `family_of`. Docstring „NOTHING IS PAIRED“
umgeschrieben (wir paaren nicht, wir lesen). *(3)* `load_budget` + Fensterfelder (gleiche Rechnung, eine
Stelle `_window_rows`), `window_free` für das Urteil (kein Rechnen im Panel); `window_sessions` mit Rest
„ohne Einheit“, Summen je Gruppe, `mismatch` bei ctlLoad < Aktivitäten. *(4)* `load_ceiling.bound_by`
(Gleichstand → Woche). `coach.today` trägt `week`; der Modus heute/morgen kommt aus `_ceiling_budget`,
derselben Stelle wie `session_ceiling` (herausgelöst, bitgenau). *(5)* Panel `_weekBox`/`_weekList`,
Variante C, Farben aus FAM/C, Schraffur `.hatch`. Die Regel `.tstate{display:flex}` aus dem Trainer-Abschnitt
griff auf den Heute-Kopf durch — mit `.tcard .tstate{display:block}` übersteuert, nicht entfernt (Befund).
*(6)* Satz coach.py „weicht/weichen ungünstig … ab“. Nichts nach intervals.icu geschrieben (E4 offen).
Rot vorher je Schritt (8 / 11 / 20 / 45); Mutationen über Dateikopie 35, alle rot.

**Einundsiebzigster Fall (0.73.1 — das Wochenziel folgt dem Zustand, Bänder am Ziel, „fällt aus dem Fenster“, Fenster beschriftet; Skizze 0.73.1).**
Befund aus der Abnahme 0.73.0 (B1–B4): Kopf „grün / Alles möglich“ aus coach.state, der Faktor aus
analytics.readiness().overall — eine zweite Ampel aus sieben Punkten, rot schon bei zwei gelben, mit
dem ACWR darin (eine volle Woche senkte ihr eigenes Ziel). Bänder fest bei 0,8/1,3/1,5, Zielstrich
wanderte: 278 bei Ziel 180 stand in „passt“ neben „Woche voll“. *(1)* `coach.BUDGET_LIGHT` (Setzung)
und `coach.week_budget` als EIN Weg; `_ceiling_budget(data, state, day)`, `session_ceiling(data, state,
day)`, `today(data, events, day)`; websocket workouts/goal/today rufen readiness nicht mehr;
`readiness` ohne Feld `budget` (kein Leser mehr: Panel las es nur in `bullet()`, ohne Aufrufer).
*(2)* `_weekBox`: drei Flächen am Ziel (passt/über Ziel/zu viel), Randfall zwei; Fußzeile mit
Zustandswort. *(3)* `drops_next` = ältester Fenstertag mit Last + `leaves_on`; Satz „Ab morgen / Ab
{Wochentag} fällt die Fahrt vom … aus dem Fenster.“ in beiden Modi. *(4)* Spanne in der Überschrift,
Morgen-Wortlaut mit Hinweis aus `window_before`, Liste „in diesem Fenster“. Wirkung: in den Prüfdaten
(`night_history`, Zustand ready, zweite Ampel gelb) steigt die Obergrenze der Trainer-Karten von 45
auf 108. Rot vorher je Teil; Mutationen über Dateikopie 17, alle rot.

**Zweiundsiebzigster Fall (0.73.2 — ein bewerteter Tag für alle Zähler, ehrliche Beschriftung des Wochenplans; Skizze 0.73.2).**
Korrektur zu Skizze 0.73.0 §13: die Einheiten des Wochenplans tragen keinen Tag; websocket_goal bewertet
jede so, als wäre sie die nächste Fahrt — kein Fenster je Tag gebaut, stattdessen beschriftet. Befund B5:
`_hard_days_recent(data, 7)` zählte immer ab dem letzten Wellness-Tag (Fenster von heute), auch wenn
Karten und Budget nach dem Training für morgen galten; live 26.09. 2 statt 1, harte Familien für morgen
herabgesetzt, Wortlaut „in dieser Woche“ falsch (rollierend gezählt). recovery_offered rechnete einen
dritten chronischen Schnitt. *(T1)* `coach.judged_day` (aus `_ceiling_budget` herausgelöst);
`_hard_days_recent(data, 7, day)` im Fenster [day−6, day], alle Aufrufer (websocket workouts/goal ×2,
assessment, recovery_offered) übergeben den bewerteten Tag; recovery_offered: ruhige Tage vor dem
bewerteten Tag, chronischer Schnitt aus dem Budget (`_ceiling_budget` → week_budget). *(T2)* Texte „in den
sechs Tagen davor / vor heute|morgen“. *(T3)* Morgen-Augenbraue, „Keine Fahrt in diesem Fenster.“
*(T4)* Wochenplan-Zeile „Bewertet für heute|morgen …“, Modus aus `_judgedTomorrow` (dieselbe Stelle wie
die Stufenworte). Rot vorher je Teil; Mutationen über Dateikopie 14, alle rot (eine erst nach
geschärfter AST-Prüfung).

**Dreiundsiebzigster Fall (0.73.3 — Aufräumen und ein veralteter Satz im Wochenplan; Skizze 0.73.3).**
*(§1, B6)* `week_done.note` sagte seit 0.73.0 das Gegenteil des Heute-Kopfs („eine automatische Zuordnung
wäre eine Behauptung …“); jetzt „Die Familie einer Fahrt kommt aus deinen Marken oder aus der Paarung in
intervals.icu; geraten wird nichts.“ `paired: False` bleibt. *(§2)* Je Stelle Leser per grep belegt, dann
entfernt: `bullet()` (Leser nur eine Testprüfung, umgestellt; Harness-Export entfernt), CSS `.bullet`,
`.bband`, `.bval` an beiden Stellen (einziger Leser `class="bval"` lag in `bullet()`), `.bmark` ohne
Elternklasse (beide Leser :1504/:1816 sitzen in `.bbar`, `.bbar .bmark` setzt alle Eigenschaften),
`.tstate{display:flex}` (Trainer ohne Leser) samt Übersteuerung `.tcard .tstate`; `weekly_load` samt
Rechnung in `_state_for_plan` (kein Leser in plan.py oder Panel); Parameter `today` von `readiness`
(zwei Testaufrufer umgestellt). Vorher/nachher: HTML der Leser per Momentaufnahme gleich; im Browser
alle 171 Elemente an gleicher Stelle (1060/700 px), einzig die wirkungslosen Flex-Werte an `.tstate`
fort. Mutationen über Dateikopie 11 (je Stück zurückgelegt), alle rot.

**Vierundsiebzigster Fall (0.73.4 — E4: feste Kennung an eigenen Workouts; Skizze 0.73.4, Freigabe Johannes 26.09.).**
Die Zuordnung einer gefahrenen Einheit zu ihrer Familie hing am Altbestand-Weg (Hinweiszeile + Katalogtitel)
und ging beim Umbenennen in intervals.icu verloren. *(§3.1)* `workouts.to_event` setzt
`external_id = "ha-intervals-icu:{key}:{day}"`, ohne Schlüssel kein Feld; sonst Payload unverändert (kein
upsert, api.py unverändert, Wächter „%“ davor). *(§3.2)* Rundlauf to_event → `_planned_key` über jeden
Katalogschlüssel samt Stufentest, roh und skaliert. *(§3.3)* `plan_workout` antwortet zusätzlich mit
`external_id` und `external_id_confirmed` (nur gleiches Echo in einer dict-Antwort; Liste/Text/None →
nicht bestätigt, kein Absturz); Knopftext „im Kalender: {Tag} · mit Kennung“ nur bei echtem `true`, sonst
„· Kennung nicht bestätigt“. Ob intervals.icu das Feld speichert und nach dem Paaren behält, ist nicht
dokumentiert: die Live-Probe (Skizze §6) entscheidet. Rot vorher je Teil; Mutationen über Dateikopie 4 aus
§7 plus 2 Gegenproben am Handler, alle rot.


**Fünfundsiebzigster Fall (0.74.0 — Belastungs-Reiter neu, Variante A; Skizze 0.74.0, Wahl Johannes 26.09. 15:46).**
Der Reiter zeigte eine ACWR-Kurve mit dem 4-Wochen-Schnitt INKLUSIVE heute, Heute-Kopf und Trainer rechnen mit dem Schnitt
VOR heute - zwei Wege zur selben Frage, und keine Verbindung zum Heute-Kopf. *(§3.1)* EIN Erzeuger je Größe:
`load_budget(planned=…)` (ohne planned bitgenau, Prüfsumme), `sessions_between` aus `window_sessions` gelöst (bitgenau),
`weeks_by_group`, `window_history`, `window_projection`, `planned_loads`; `coach.load_view` verdrahtet nur (Licht je Tag aus
`state_series`, Vorschau mit dem Licht von heute, Überschrift = `today().week`); `websocket_load` = summary + load_view.
readiness „Akut zu chronisch“ = window_load/(7 × chronic) aus load_budget (Beispiel-Fixture 1,34 → 1,51, gelb → rot);
summary ohne `acwr`, `acwr_latest`, `thresholds.acwr_*`. *(§3.2/3.3)* Oben „Deine Wochen“ (12 KW, gestapelt nach Gruppe,
geplant gestrichelt), dann „Die letzten 7 Tage im Verlauf“ mit Vorschau im eigenen Scrollrahmen. Rot vorher je Teil
(Backend an 0.73.4 17 + 1 + 3 rot, Panel an 0.73.4 53 rot); 6 Mutationen aus §8 über Dateikopie, alle rot.
**Offen (gemeldet, nicht mitrepariert):** `coach.signals` liest weiter `acwr_series` (Signale-Reiter, Schnitt inklusive
heute) - der letzte zweite Weg zum Verhältnis; `_sparkFor("acwr")` im Panel hat keinen Aufrufer und liest das entfernte
`load.acwr`; der readiness-Punkt wird im Panel nirgends gezeigt (`rTrainer` nimmt `rd`, liest es nicht).

**Sechsundsiebzigster Fall (0.74.1 — Feinschliff nach der Live-Abnahme; Skizze 0.74.1, 26.09.).**
*(B1)* `monthTicks` (7 Aufrufer: Signale, Fitness, Belastung ×3, DFA, Test) beschriftete „07.26 08.26“ - las sich wie ein
Datum, und der Rest eines angebrochenen Monats klebte am ersten vollen. Jetzt Monatsname („Aug“), Jahr nur beim Jahreswechsel
und am ersten Tick eines Diagramms über einen Jahreswechsel („Jan 27“); ein Tick näher als 6 % der Indexbreite (n − 1) am
nächsten entfällt, der frühere (`TICK_MIN_GAP_PCT`, Setzung der Skizze). *(B2)* „Vorwoche {p} Last – {±z} % gegenüber der
Woche davor.“ statt „(+41 %)“ (der Leser bezog die Zahl auf diese Woche). *(B3)* Satz zur Vorschau: je Fall genau einer, zwei
Fälle hatten keinen, der Fall „dazwischen darüber“ bekam nur „wieder unter“. *(B4)* EIN Weg zum Verhältnis:
`analytics.window_ratio(data, day)` = window_load / (7 × chronic) aus `load_budget(today=day)`; readiness und
`coach.signals` rufen sie; `acwr_series` samt Tests entfernt (Leserliste danach leer). Beispiel-Fixture test_coach vorher:
readiness 1,55 gegen Signale 1,44 am selben Tag. *(B5)* `_sparkFor` ohne Aufrufer entfernt (mit ihm die Prüfung „7 eine
Einheit je Kachel“, die nur die tote Funktion prüfte). *(B6)* `planned_loads` filtert `hide_from_athlete` wie
`_day_planned`. Rot vorher je Teil (B1 9, B2 2, B3 10, B4 8 + 5, B5 2, B6 2); Seitenprobe Fitness/Belastung/Signale/DFA als
Prüfung; 5 Mutationen aus §3 über Dateikopie, alle rot. **Aus Fall 75 erledigt:** signals/acwr_series, `_sparkFor`. **Weiter
offen (gemeldet, nicht mitrepariert):** der readiness-Punkt wird im Panel nicht gezeigt; `window_ratio` nimmt für Tage mit
weniger als 28 Tagen Vorlauf den kürzeren Schnitt aus `load_budget` (die alte Reihe hatte dort keinen Wert - betrifft nur die
ersten Wochen eines neuen Bestands); `websocket_signals` rechnet als `@callback` in der Event-Loop, jetzt 180 × `load_budget`
(synthetisch 500 Tage: 0,22 s → 0,35 s).
**Siebenundsiebzigster Fall (0.74.2 — Nacht danach ohne Doppelung, 28-Tage-Regel im Budget; Skizze 0.74.2, 26.09.).**
*(B1)* Mit Etikett auf der Nacht sagten zwei Kästen hintereinander „nicht bewertbar“: der Kopf aus `coach.night_after`
(`headline`) und das fette Label der Karte `_nightVerdict` (`verdict.label`). Jetzt trägt die Karte beim Schlüssel
`nicht_bewertbar` kein eigenes Label, nur die Zeile mit den z-Werten und die Regel; der Kopf bleibt. Dieselbe Karte steht im
Heute-Reiter unter dem Nachtkasten - dort wirkt es genauso. *(B2)* Die Zeile hieß bei fehlender zweiter Nacht „zweite Nacht
zweite Nacht liegt noch nicht vor“: das Panel setzte „zweite Nacht “ vor `note`, und `note` (`NIGHT_VERDICT_WORDS["no_second"]`)
beginnt selbst so. Jetzt „Nacht danach {z1} · {note}“, mit z2 wie bisher. *(B3)* `load_budget` prüfte nur `len(series) < 28`;
ein Tag mit weniger als 28 Tagen davor rechnete mit einem kürzeren Schnitt. Jetzt zusätzlich `len(before) < 28` → None (die
Definition 7:28, keine Setzung); die alte Prüfung bleibt, damit geplante Tage nicht als Verlauf zählen. Leser (alle behandeln
None schon): `week_budget` → `_ceiling_budget`/`session_ceiling`/`today`/`load_ceiling`, `window_ratio` → `readiness` und
`coach.signals`, `window_history`, `window_projection` (über `_window_point`). Am Johannes-ähnlichen Bestand (12.05.2025 bis
26.09.2026) sind `today`, `load_view`, `signals`, `readiness`, `session_ceiling` byte-gleich zu 0.74.1; nur der linke Rand
eines Verlaufs, der in den ersten 28 Tagen des Bestands beginnt, hat dort jetzt kein Ziel (Test „0.74.0 d“ angepasst: die
ersten 18 Verlaufstage des 70-Tage-Bestands). Rot vorher je Teil (B1 3, B2 5, B3 5); 3 Mutationen aus §3 plus Grenzen 27/29
und „Label bei allen weg“ über Dateikopie, alle rot. **Aus Fall 76 erledigt:** `window_ratio` mit kürzerem Schnitt.
**Gemeldet, nicht mitrepariert:** die Fixture `F.night()` trägt noch den Regeltext von 0.69.0, nicht den des Erzeugers.
**Achtundsiebzigster Fall (0.74.3 — Heute zeigt die letzte GEMESSENE Nacht nach einer Einheit; Skizze 0.74.3 Fassung 2, 26.09.).**
`coach.today()` nahm die zeitlich letzte Aktivität (nur nach Datum, jede Art und Dauer) und fragte `night_after` für genau
diese; fehlte deren Nacht, ließ das Panel den Abschnitt **stillschweigend weg** (Live 26.09.: VO2max-Nacht vom 25. nicht
sichtbar, weil die Grundlage vom 26. noch keine Nacht hatte). Jetzt eine Stelle `coach._last_measured_night`: Kandidaten sind
Einheiten nach `_is_session` (≥ 900 s, die Grenze aus `_trained_today`, jetzt eine Hilfsfunktion; `_trained_today` bitgenau
gegen die eingefrorene Fassung) im Fenster aus 7 Kalendertagen [letzter Wellness-Tag − 6, letzter Wellness-Tag], neueste
nach voller Uhrzeit; gezeigt wird die neueste mit vorliegender Nacht (`night_after` unverändert, dazu `activity_name`,
`activity_id`), eine neuere ohne Nacht steht als `night_pending` (`pending` nach dem letzten Wellness-Tag, sonst `missing`),
ein leeres Fenster als `night_none`. Das Panel zeigt Überschrift „DIE LETZTE GEMESSENE NACHT NACH EINER EINHEIT“, die Zeile
„nach {Tag} · {Name}“ (voller Name, einzeilig, Kürzung nur per CSS, title), darüber die pending-/missing-Zeile; der Abschnitt
verschwindet nie mehr ganz (Fehlerklasse 4, „der stille Ausstieg“). Rot vorher: test_coach 23, test_panel_views 30. Sieben
Mutationen aus §8 über Dateikopie rot (1/2: 10/3, Fenster 1, Sortierung 1, pending 11, none 2, Name 12 Zeichen 6). Die
Momentaufnahme 0.73.3 (heute/morgen) neu gesetzt — belegt, dass sich die Seiten nur im Nacht-Abschnitt und der Unterzeile
unterscheiden. **Gemeldet, nicht mitrepariert:** der Streifen „Woher das kommt“ nimmt die letzten 7 Wellness-Zeilen, nicht
7 Kalendertage (Restliste); `coach.py:399` führt dieselbe 900-s-Grenze ein zweites Mal (`< 900`), liest `_is_session` nicht.

**Neunundsiebzigster Fall (0.74.4 — Die Nacht in Klartext; Skizze 0.74.4, Wortlaut von Johannes 26.09. 18:26 freigegeben).**
Johannes 18:24: „auf welchem Planeten muss ich leben, um den Text zu verstehen?“ Im Nacht-Abschnitt (Heute und Aktivitäten)
standen „SD“, „Gewicht 0,5“, „misst nicht nur die Einheit“, „Erholung, nicht Bereitschaft“, „… −3,1 SD davon“ und ein
Regelsatz voller Schwellen; dazu zwei Urteile ohne Vergleichsbasis, die sich zu widersprechen schienen. Reiner Text-Release,
keine Regel, keine Schwelle, keine Zahl bewegt (Fingerabdruck der Zahlen und Urteile bitgenau gegen 0.74.3). Neu: `coach.z_word`
als die EINE Wortstufe (halboffen an SWC_SD / neuer `DAY_SWING_SD` = 1,0 „gewöhnliche Tagesschwankung, kein neuer Wert“ /
HRV_DROP_SD; unter/über aus Rohwert gegen Basislinie), Worte je Werte-Zeile, an der Referenz (`_reference_words`, Seite aus
Richtung × mittleres z) und an beiden Nächten der Karte; `_night_label` liefert Tag und Etikett einzeln; `night_rule_text()`
baut die Regel aus NIGHT_TOO_MUCH_Z / NIGHT_DIGESTED_Z / DAY_SWING_SD. Panel: Frage-Zeile statt Großbuchstaben-Kopf, Karte
„HRV in der Nacht danach: {Wort} ({z}) · Zweite Nacht: …“, Regel zugeklappt unter „Wie wird das bewertet?“, Werte-Zeile
„{Wort} ({z}) · nach solchen Einheiten sonst: {Wort} ({Mittel})“ ohne „davon“. Rot vorher: test_coach 90, test_panel_views 61,
test_zweiter_athlet 5. 13 Mutationen über Dateikopie rot (Wortstufe im Panel 6, Grenze 1,0 als Literal in z_word 1 / im
Regeltext Backend 1 / Panel 6, Label bei nicht_bewertbar 7, Regel sichtbar 11 / offen 6, unter/über aus z 8+1, Referenz aus z 3,
alter Zweite-Nacht-Satz 2+1, Gewicht im Satz 9, alte Überschrift 10, davon-Zahl 4). Momentaufnahme 0.73.3 neu gesetzt — belegt,
dass Heute/Morgen sich nur im Nacht-Abschnitt und der Unterzeile unterscheiden. **Seitenprobe: zwei Widersprüche aus der Skizze
selbst, vorgelegt, nicht drumherum gebaut:** (1) die Wortstufe liegt auf dem genauen z, die Zahl steht mit einer Nachkommastelle —
an jeder Grenze ±0,05 widerspricht die Klammer dem Wort und der Regel („im Normalbereich (-0,5)“ neben „ab 0,5“); (2) „Wie das zu
lesen ist“ sagt weiter „gegen deine eigene übliche Antwort …, nicht gegen einen Normwert“, die Karte urteilt gegen „deine normalen
Nächte“ — §3.4 sagt nur „angleichen“, ohne Wortlaut. **Gemeldet, nicht mitrepariert:** „Zweite Nacht: kommt morgen“ steht auch an
alten Einheiten, deren zweite Nacht keine HRV hat; bei „unknown“ wiederholt das detail den neuen Satz; die Zahl nutzt das
ASCII-Minus des gemeinsamen Formatierers (Skizze: „−“); die Signal-Kacheln oben tragen weiter „SD“ (§4, eigener Durchgang);
`test_workouts`-Wächter sammelt `entry.*` aus dem ganzen Panel (Nacht-Felder auf seiner Ausnahmeliste).
---
...
**24 Dateien, 9.711 gezählte Einzelprüfungen, alle grün.** Kein Test braucht eine laufende
...
| `test_import.py` | vollständiger Import gegen einen Nachbau des Kontos, Schwellenreihe und `since`, day_context-Migration und Schreibweg, Quellenblock-Auflagen, **der Versionsmarken-Wächter mit Gegenprobe über eine dritte Marke, der Historienbeginn getrennt vom Bestandszeitraum** ; **seit 0.63.1 die ausgefallenen Stromabrufe: beide werden namentlich mit ihrem Grund gemeldet, die geglückte Fahrt nicht, Gegenprobe ohne Ausfall, und der Wiederholversuch gibt nur die leeren Zeilen frei, lässt Gemessenes stehen und ist beim zweiten Mal ein No-op** ; **der `except`-Zweig am ECHTEN Lauf durchlaufen: der Fake-Client wirft für eine Fahrt, die Meldung nennt sie mit Grund und Datum, die leere Zeile bleibt als Marke stehen, sie wird nicht wieder angefragt — mit der Gegenprobe, dass die geglückten Fahrten nicht im Fach stehen** ; **seit 0.66.3 (F1.5) prüfen `drop_outdated_dfa`/`needs_activity_refetch` auf ÄLTER: eine neuere Marke wirft nichts weg und wird nicht heruntergeschrieben, ältere/fehlende/None weiter verworfen — rot an 0.66.2 vor dem Bau** | 152 |
| `test_analytics.py` | Trainingsmetriken gegen bekannte Ergebnisse, Ebene-3-Wächter (Last kennt keine Etiketten, Quelltext und Verhalten), **die Wochenbilanz aus dem Archiv — Abgrenzung, und dass nichts gepaart wird** ; **seit 0.67.2 (S2) ist die Kalender-Wochenlast die Summe der Tageslasten aus `load_by_day`, mit Fixture-Beweis gegen die Aktivitätssumme** ; **seit 0.67.3 (F2.10) zählt `load_budget` die sechs Tage VOR heute und beziffert die heutige Last als `used_today`: Bestand mit heutiger Fahrt → Budget unverändert, Verbrauch 60; ohne → 0; Randfall ohne heutige Zeile; Mutation „heute wieder abziehen“ 4 rot** ; **seit 0.67.4 (F2.9) stuft `form_state` absolut, Belastungs-Reiter und Ampel lesen ihn (+6,7 → grey/green statt 21,5 % → transition/amber) — Mutation „wieder relativ“ 3 rot** ; **S1 (0.69.0): `hrv_status` liest das eine Band (`baseline.band_before`, 60 Nächte davor, gewichtet), SWC = 0,5 × Streuung der Nächte, flache Reihe → kein Band; gewichtet sieht den Einbruch, ungewichtet nicht — Ampel-Komponente und Vorbemerkung (`context_note`) mit Zahlen des Trainers; Ebene 3 nur noch für die Last (Budget je Ampelfarbe kontextfrei)** ; **seit 0.73.0 `activity_family` je Weg (Marken, mehrere Familien → härteste, external_id, Altbestand mit Hinweiszeile + Katalogtitel, fremde/unbekannte external_id, umbenannter Titel, ins Leere, Pendelfahrt, andere Sportart, kein Namensvergleich, schreibt nichts), die Fensterfelder von `load_budget` (Start/Ende, Last, erlaubt, frei, Bänder, drops_next, Ampel gelb/rot, alte Felder unverändert) und `window_sessions` (Summe = window_load, Rest, Reihenfolge, Gruppen, Lücke, leer, unter 28 Tagen, Athlet B, ctlLoad < Aktivitäten benannt)** ; **seit 0.73.1 drops_next = ältester Fenstertag MIT Last samt leaves_on (0 übersprungen, keiner → None, nur heute), window_before aus derselben Reihe; readiness ohne budget, zwei Prüfungen vom Weg „Ampel → Faktor“ auf load_budget direkt umgestellt** ; **seit 0.73.3 der Satz in week_done wörtlich (alter fort, paired False), readiness ohne today; F2.9-Aufruf umgestellt** ; **seit 0.74.0 (Skizze 0.74.0 §3.1) `load_budget(planned=…)`: ohne planned bitgenau (Prüfsumme an 0.73.4), mit planned zählt die geplante Last wie gefahren und die Reihe läuft lückenlos bis zum Tag; `sessions_between` herausgelöst, `window_sessions` bitgenau (Prüfsumme); `weeks_by_group` (Mo–So, Summe der Gruppen = Tageslast, Rest in none, Fehlstelle benannt, geplant nur laufende Woche nach heute, KW 53/Jahreswechsel, zweiter Athlet); `window_history`/`window_projection` je Tag aus load_budget mit dem Licht des Tages bzw. von heute, unter 28 Tagen ohne Ziel; `planned_loads` (WORKOUT, > heute, Last > 0, ungepaart); readiness „Akut zu chronisch“ = window_load/(7 × chronic) aus load_budget (Beispiel 1,34 → 1,51); summary ohne acwr/acwr_latest/thresholds.acwr_*; Mutationen planned ignoriert 7, gepaart doppelt 3, readiness mit acwr_series 3 rot** | 289 |
| `test_setup_simulation.py` | Entity-Aufbau, Übersetzungen, unique_ids | 22 |
| `test_laps.py` | Runden-Normalisierung | 34 |
| `test_coach.py` | Zustandsregeln, Trigger-Schärfung, Infektverlauf, Nachtreaktion, Einordnung, Bereiche, benannter 42-Tage-Verlauf, Basislinien-Primitive mit AST-Wächter, eingefrorene No-op-Referenz, gewichtete Basislinie mit Fixture-Beweis, **Durability: die drei Ehrlichkeitsregeln einzeln, Gewichtungs- und Umrechnungs-Gegenprobe, Blockverlauf; der Kopf: belegte Dauer am gesperrten Fall, längste ≠ arbeitsreichste und Fahrt-Watt ≠ Pool-Median je erzwungen, Fensterausweitung mit Gegenfall, Progressionsfaktor mit 1,0-Gegenprobe**; **die Erholungs-Setzung hinter der Reiz-Stufe** ; **seit 0.67.2 (S2) lesen Heute-Streifen, Signale-ACWR und Kalender-Woche den einen Erzeuger `analytics.load_by_day` (ctlLoad, sonst Aktivitätssumme des Tages) — Fixture mit absichtlich verschiedenen Reihen, drei Mutationen rot** ; **S1: `_band` nur noch in `night_after`, `_norm_band`/`_z_series` reichen an baseline.py durch; Signale gewichtet, gleich dem Heute-Reiter; Fenster der Anzeige-Bänder VOR dem Tag** ; **L2 (0.69.0): `night_verdict` — verdaut ≥ −0,5 · gekostet < −0,5 · zu viel < −1,0 oder zweite Nacht < −0,5, Schwellen benannt, Setzung beschriftet, zweite Nacht als offen benannt; Wächter: `state`, `state_series`, `load_ceiling`, `recovery_offered`, `assessment`, `coach` und workouts.py lesen die Bewertung nicht** ; **seit 0.69.1 die geschärfte L2-Regel: Wahrheitstafel je Zweig mit Rändern, zweite Nacht allein = gekostet verzögert, Livewert 01.09. als Sollwert** ; **seit 0.69.2 (F2) `session_ceiling`: nach dem Training die Grenze von morgen, vorher bitgenau `load_ceiling`, unter 28 Tagen kein Budget; AST-Wächter auf beide Handler** ; **seit 0.70.0 (C4) die Erholungsregel sagt bei der Grenze 0 „kein harter Tag“ statt „höchstens 0 harte Tage“; der Kurzsatz `short` des Zustands (A2) benannt geprüft und aus der Prüfsumme genommen, die alte Summe hält weiter** ; **seit 0.72.1 die Nacht mit Etikett: erste oder zweite Nacht mit Gewicht < 1 → „nicht bewertbar“ mit Grund-Satz an L2 UND am Vergleich, Rohwerte bleiben; Gegenproben ohne Etikett bitgenau wie 0.72.0 (Prüfsumme), Normal, Gewicht 1,0, Etikett am Tag der Einheit; Peers mit Etikett fallen aus der Referenz, unter fünf „kein Vergleich möglich“; Etikett Cannabis mit Beleg und Setzung, die alten Etiketten per Prüfsumme unverändert** ; **seit 0.73.0 `bound_by` je Zustand samt Gleichstand und ohne Budget, Rechnung der Obergrenze unverändert; die Woche im Heute-Payload (week_load == window_load, Modus heute/morgen aus derselben Stelle wie session_ceiling, ohne Budget die Liste); der Satz „weicht/weichen ungünstig … ab“ mit unverändertem Rest** ; **seit 0.73.1 BUDGET_LIGHT (Setzung, neben CAPACITY), je Zustand der Faktor über den einen Weg (week_budget), Live-Fall readiness rot + ready → 1,3, Trainer-Grenze = Heute-Grenze je Zustand, AST: Handler und coach ohne readiness()/overall, genau ein load_budget-Aufruf in coach; 18c F2 und alle today(budget)-Aufrufe umgestellt** ; **seit 0.73.2 judged_day als eine Stelle, harte Tage im Fenster [Tag−6, Tag] (Live-Fall 2 → 1, Grenzfall Tag−6/Tag−7, Obergrenze des Fensters, heute-Modus wie 0.73.1), AST: jeder websocket-Aufruf übergibt den Tag aus judged_day; recovery_offered mit week_budget-chronic und ruhigen Tagen vor dem bewerteten Tag; Texte „in den sechs Tagen vor heute|morgen“** ; **seit 0.74.0 `load_view`: Verlauf je Tag mit BUDGET_LIGHT[Zustand des Tages] (Einbruch → ×0,8, fehlend → unknown), Vorschau mit dem Licht von heute und dem Plan aus den Events, Überschrift = today().week in drei Fixtures (ohne Fahrt, Einbruch, mit Fahrt), frei = Trainer-Obergrenze (§5); AST: keine eigene Rechnung; Regel 9 hält die Schlüssel der Panel-Fixture gegen den Erzeuger; Mutationen Faktor fest 1,3 4, Überschrift eigene Rechnung 3, gepaart doppelt 3 rot**  ; **seit 0.74.3 die letzte gemessene Nacht im Heute-Payload (`_last_measured_night`): Fälle A (Live), B (vor dem Abgleich), C (Spaziergang, 899/900 s), zwei am Tag (Uhrzeit, beide Speicherreihenfolgen), Tag −7/−6, missing mit Gegenprobe „nur HRV fehlt“, Etikett, zweiter Athlet, Name aus Sportart; `today().night` ohne die Kopffelder bitgenau `night_after`; `_is_session` eine Stelle, `_trained_today` bitgenau gegen die eingefrorene 0.74.2-Fassung; vier Mutationen rot** ; **seit 0.74.4 `z_word` als die eine Wortstufe (Grenzen 0,49/0,5/0,99/1,0/1,99/2,0 gespiegelt, Konstanten SWC_SD/DAY_SWING_SD/HRV_DROP_SD verschoben → Stufe folgt, unter/über aus dem Rohwert samt Ruhepuls, Rückfall Vorzeichen); fünf Vergleichssätze und fünf Karten-Labels wörtlich an echten Daten, beide Vergleichsbasen, „Zweite Nacht: kommt morgen“, Etikett-Satz aus Tag und Etikett, Regel wörtlich mit Zahlen aus den Konstanten, keine Fachwörter; Zahlen-Fingerabdruck bitgenau wie 0.74.3; Regel 9: Worte, Labels und Regel der Panel-Fixture gegen den Erzeuger; 0.72.1-Prüfsumme und sechs Wortlaut-Prüfungen umgestellt** | 789 |
| `test_plan.py` | Zielprofil, Wochenmuster, Zeitbudget, Progressions- und Kalender-Anker-Vertrag, Profil-Migration ; **seit 0.72.0 (C5, Wahl 1) der Deckel des großen Tages aus `state.progression`: gedeckelt mit Trefferzusicherung, Wochenstunden folgen, Rechenweg-Feld `big_day_cap`; Gegenproben Grenze über dem großen Tag, ohne Progression, tiefe Grenze unter dem Routinetag; plan.py kennt den Faktor nicht, `_state_for_plan` fragt coach** | 417 |
| `test_workouts.py` | Einheitenauswahl, HF-Klemme, Infektleiter, Wattumrechnung, Intervals-Syntax, **die vier Stufen über die volle Wahrheitstabelle, rot mit Begründung welches von beiden, dieselbe Einheit über alle vier Stufen, die Lastskalierung an den Zahlen des Livebestands, eine Zustandsregel für beide Ansichten, kein Urteil ohne Stufe**; **die Staffelung aus der Kurve: gepaarte Reihe, Studienform darüber, Ein- und Ausrollen bleiben FTP, harte Familien unberührt**; **die Vorgabe ist ein Anteil der Schwelle, nicht die Schwelle — Watt- und Pulsseite gegeneinander gehalten** ; **Watt und Puls der harten Familien aus DERSELBEN Quelle, mit dem umgeschriebenen Gleichstandstest: fällt eine Seite auf die FTP zurück, muss die andere mitfallen**; **seit 0.51.0 die Quellenkette je Familie als EINE Tabelle: Rangfolge am Ergebnis von `scaled()` erzwungen, jede Stufe beschriftet einschließlich des Rückfalls, der Gleichstand über den Stufentest hinweg, und der LEERZUSTAND Zahl für Zahl — ohne markierten Test entscheidet die Kette wie zuvor** ; **seit 0.56.1 die Wattliste auf JEDEM Weg: Blöcke, Kurve, beide Stufentest-Schwellen und FTP, mit Trefferzusicherung, dass jede Stufe durchlaufen wird, und ohne FTP keine halbe Wattliste** ; **seit 0.58.0 nennt der Stufentest je Zahl seine Auswahl, neben dem Messweg und nicht in SOURCE_LABEL** ; **seit 0.59.0 die Kachel-Erklärung (B2c): Stufe im Kreislauf je Quelle UND Auswahl, genau eine Stufe „hier", gewertete Einheiten aus dem Zählfeld (absichtlich abweichend von der Liste), der Rechenweg endet auf der Vorgabe, die FTP wird übergeben statt aus gerundeten Watt zurückgerechnet, der Stufentest bekommt keine zweite Herleitung** ; **seit 0.67.2 (S4/F3.3) trägt eine gestreckte Wocheneinheit in `text_w` die Wattliste von `blocks_w` (130, nicht 134 W) — rot vorher, Gegenprobe ungestreckt** ; **seit 0.67.4 (S3) liest `curve_watts` die Kette der Kachel bei der geplanten Dauer (volle Stunden, `planned_hour`), Schranke `CURVE_HOUR_MIN_RIDES` = 3, jenseits des Belegten die letzte gut belegte Stunde — Livezahlen als Sollwerte (3,5 h → Stunde 3, 126 W); drei Mutationen (Stundenmitte 4, Schranke weg 5, Rundung halb-auf 4) rot; die alten Kurven-Wächter auf Kette A nachgezogen** ; **seit 0.68.0 liest die Grundlage die Umkehrung (`ga_at`, Ziel/Grenze aus `ga_targets`), die 0,90 ist weg, ohne Ziel trägt sie die Grenze, ohne Stufentest die FTP mit Grund (`ga_missing`), ab 3 h geplant `unverified` (benannte Schwelle `GA_UNVERIFIED_FROM_HOUR`, auch beim Rückfall auf eine frühere Stunde); die alten Kurven-Wächter (Studienform, 0,90) nachgezogen** ; **seit L1 (0.69.0) die Wahrheitstabelle nachgezogen: die Stufe folgt dem Zustand, über der Obergrenze `over_ceiling` statt rot, `guard()` mit Last/Obergrenze/passender Dauer (elastisch 5 h × 200/250 = 4,0 h; feste Einheit „nicht kürzbar“), ohne Zustand (`by_load`) rot am Budget mit Beschriftung; sechs Mutationen rot** ; **seit 0.69.1 das Wort der Stufe über der Obergrenze aus `GUARD_WORDS["over_word"]`, grün wie gelb, im Budget unverändert** ; **seit 0.69.2 (F2) `guard()`: „nicht kürzbar“ nur für feste Einheiten, elastische/Regeneration gegen 0 sagen „auch gekürzt“** ; **seit 0.70.0 C4 (Stufentexte auf L1: kein „Budget“ in grün/Reiz/rot, gelb wortgleich), C7 (Block 5 einer 5×4 ist Arbeit — Steuerungs- und Blockzweig, Gegenproben Pause/Satz/30/30), C8 (Regeneration nie über der Grenze: guard, suggest, Woche; Wiedereinstieg bleibt drin); zwei L1-Prüfungen und die F2-Regenerationsprüfung auf C8 umgestellt** ; **seit 0.72.0: jede Variante einer Familie aus derselben Stelle wie die gewählte (`judge` in `suggest`), mit Trefferzusicherung an der Obergrenze und der Gegenprobe Familie mit einer Einheit; drei neue Einheiten (Familie, Blöcke, Wattquelle, Setzung HF/Laktat → Watt, Neal-Zusatz, keine Bestandszahl); die Beleg-Kennung je Karte genau nach der Tabelle der Skizze, alte Belegtexte erhalten** ; **seit 0.72.2 das Stufenwort nie ersetzt (Worte der Entscheidung 26.09., `{tag}` für heute/morgen), das Mengen-Zeichen `quantity` nur bei grün/gelb über der Obergrenze — Gegenproben unter der Grenze, ohne Budget, Reiz, rot, `by_load`, Regeneration und Grundlage bei 0; Rechnung (Stufe, Budget, guard) unverändert; `over_word` entfernt; drei Prüfungen der 0.69.1-Regel und C8 auf das Zeichen umgestellt** ; **seit 0.73.0 die eine Tabelle Katalogschlüssel → Gruppe (`KEY_GROUP` aus `FAMILY_GROUP`), Härte-Reihenfolge als Setzung E3, am Quelltext gleichgehalten mit `TRAINER_FAMILIES` im Panel** ; **seit 0.73.2 „Standard für Wochen“ → „Zwei in sieben Tagen sind der Standard“ (umgestellt)** ; **seit 0.74.4 `word` auf der Ausnahmeliste der Karten-Felder (ein Feld der Nacht-Werte-Zeile, wie label/unit/value/z)** | 2155 |
| `test_websocket_registration.py` | Registrierung, Dekoratoren, FTP-Quelle, eine Ankerregel, day_context-Lese/Schreibweg, Ampel-Herkunftsnotiz, **der goal-Handler verdrahtet nur und bewertet ausschließlich die laufende Woche**; **`blocks` gehört zu den Payloads, ohne die eine Vorgabe still zurückfällt**; **seit P3 die drei Zuordnungswege samt FEHLERPFAD: in keinem Fehlerzweig des Schreibwegs wird geschrieben oder gespeichert, jeder bricht ab — am Syntaxbaum geprüft und mit eingebautem Schreibvorgang gegengeprobt; die Rücknahme holt keine Abschnitte; Familien und Driftsätze kommen aus dem Modul**; **seit B1 der Messweg: ZWEI Abrufe aus den Kanälen des Importwegs, Drift und Maske VOR der Rechnung, kein Fehlerzweig misst oder speichert — mit der Gegenprobe, dass der Sachbefund sehr wohl geschrieben wird; und `websocket_laps` als Schreiber, dessen Speichervorgang am Änderungsfall hängt, am Syntaxbaum geprüft**; **seit 0.54.1 die Familienreinheit: die Maske nimmt nur die Marken EINER Familie, und jeder familienlose `marked()`-Aufruf im Produktivcode ist einzeln benannt statt gemustert**; **seit B2b-0 misst der Knopf ALLE markierten Familien, jede mit ihrem Instrument — und die Blockzeilen werden FRISCH gerechnet: drei Wächter belegen, dass er nicht ins Archiv greift** ; **seit 0.63.1 der Trockenlauf und der Wiederholversuch als eigene Befehle** ; **das Befehlsmuster kennt Ziffern (`[a-z0-9_]+`), sonst fände es `set_v2_…` nie** ; **seit 0.65.1 am Syntaxbaum: der Messweg läuft über ALLE Familien und ruft `set_measurement` genau einmal, INNERHALB dieser Schleife — eine Fahrt mit zwei Marken darf nicht mit einer Messung zurückbleiben** ; **seit 0.73.1 goal ruft session_ceiling statt readiness (umgestellt)** ; **seit 0.73.3 Plan-State ohne weekly_load (samt Rechnung), gelesene Felder bleiben; „wochenlast aus Aktivitäten“ auf „Stunden aus Aktivitäten“ umgestellt** | 510 |
| `test_reconcile.py` | Abgleich mit Intervals: die drei Sperren einzeln, die datumslosen Aufräumstellen, No-op ohne Speichervorgang, der Handler am echten Aufruf (Import läuft, Historie nie geholt, Zwischenstand) ; **seit 0.66.3 (F1.6) räumt `apply` alle id-geschlüsselten Blöcke (`ID_BLOCKS` als festgehaltenes Register: activities, dfa, dfa_failed, ramp_tests, section_marks) und `plan` sagt Marke und Stufentest vorher an — mit der vorhandenen Einheit als Gegenprobe, rot an 0.66.2+ vor dem Bau** | 143 |
...
### VORLAGE C5 (0.70.0) — „lange Fahrt bis 3 h 50“ neben „großer Tag 6,0 h“

> **Entschieden 26.09.2026: Wahl 1, gebaut in 0.72.0** (§7 Fall 66) — der große Tag ist durch
> `coach._progression` gedeckelt. Der Text darunter ist die Vorlage, wie sie stand.

Zwei Regeln sagen in derselben Woche Verschiedenes, und keine nennt die andere:
`coach._progression` (Z. 593 ff.): längste gleichmäßige Fahrt der letzten 30 Tage (weitet auf
90/365/alle) × `PROGRESSION_FACTOR` 1,10 — „bewusst dieser Zeitraum und nicht deine Bestleistung“.
`plan.py` (Z. 290–316): der große Tag startet an der längsten Fahrt des GANZEN Archivs
(`longest_now`) und wächst je Zyklus × `BIG_DAY_STEP` 1,12 bis zum Ziel; der Budget-Satz
(Z. 360 ff.) nennt dazu die Zielfahrt. Welche Zahl live die „6,0 h“ ist (Wochenwert `long_day_hours`
oder Zieldauer im Budget-Satz), ist am Livebestand zu messen. Wahl für Johannes: (1) der große Tag
wird von der Progressionsgrenze gedeckelt (ein Erzeuger; bewegt die Wochenstunden — Vorgabe);
(2) beide bleiben, die Progressionszeile sagt „gilt nicht für den großen Tag, der ist die geplante
Ausnahme“ (nur Text); (3) wie heute, nur im Rechenweg nebeneinander erklärt. Nicht gebaut.
...
steht die Vorgabe still, egal wie sich die Form bewegt.

Synthetik, fünf Seeds, Welt mit Formsprung Watt +20 bei alpha im Korridor: die Vorgabe
bleibt auf 250 W (`moves = 0`), der Abstand zum gefahrenen Median wächst auf **24–34 W**.
GEGENPROBE mit alpha ausserhalb: `moves = 3`, Vorgabe 265 W — und der Abstand wächst auf
**39–49 W**, weil der Regler auf den **Startwert** rechnet, nicht auf die Leistung.

Am Livebestand heute ist der Abstand 6,2 W (VO2max 250 W gegen Fenster-Median 243,8 W) und
stammt **nicht** vom Regler: `n_since = 0`, `moves = 0`, und die `sides` aller Einheiten
sind `0`. Er ist Startwert gegen Fenster, sonst nichts.

**Das ist kein Fehler des Bandes und keine Nebenwirkung der Fenstergrösse.** Es ist die
Bauart, und sie gehört als eigene Runde entschieden — nicht als Nebensatz einer anderen.
Rechenweg und Zahlen: `docs/rechenwege.md` K8.4 und K10.2.
...
Zehn Regeln: fünf aus den Auslieferungen **0.63.0 bis 0.65.2** (19.09.2026), vier aus dem Prüfstandsbefund der Karte (22.09.), eine aus dem Michael-Befund (0.66.3). Jede steht hier, weil
...
### 6 · Eine Trefferzusicherung, die nur den Zweig trifft, den die Fixture ohnehin nimmt, bestätigt die Lücke statt sie zu finden

**Karte, Runde 5 (P1):** `test_workouts` sicherte „Tempo läuft über ftp" zu — mit einer
Blockfixture, die nur VO2max enthielt. Der Tempo-Zweig war unerreichbar, die Zusicherung grün.
Gleiche Klasse: F2.1 (`compare` nie mit einer Familie ohne Startwert), F1.8 (eine Familie),
W3.5 (drei Fixtures im einigen Bereich), F1.9 (der Wächter trug den Leerlauf als Erwartung).
Wer „für jede Familie" prüft, gibt jeder Familie Daten — und zeigt, dass der Gegenzweig
erreichbar ist.

### 7 · Ein Wächter, der „jeder" sagt, zählt aus dem Code, nicht aus einem Namensmuster

**Karte, Runde 5 (P2):** der Knopf-Wächter aus 0.63.2 prüfte Befehle, deren Name mit `set_`
beginnt — `fatigue_dry_run` fiel durch. „Warnungen müssen das DOM erreichen" galt für
`coach.warnings` — die Ampel fiel durch. `reconcile.apply` räumte „all three sites", weil der
Docstring das sagte (F1.6). Die Menge kommt aus der Registrierung, der Payload, dem Register;
nie aus `startswith` oder aus einem Satz.

### 8 · Was ein Test ersetzt, gilt als ungeprüft

**Karte, Runde 5 (P3):** `ws._max_hr = lambda data: 190.0` — die Funktion, die im Betrieb immer
`None` liefert (F1.1), wurde im einzigen Handlertest durch einen Wert ersetzt. Jede Funktion,
die ein Test ersetzt, hat eine eigene Prüfung mit echtem Eingang — sonst steht sie auf der
Liste der ungeprüften.

### 9 · Eine Fixture trägt kein Feld mit Inhalt, das der Erzeuger nicht mit Inhalt schreibt

**Karte, Runde 5 (P4):** `panel_fixtures.js` füllte `today.state_text`; `coach.today` schreibt
`condition.get("text")`, und `state()` hat keinen Schlüssel `text` — im Betrieb leer, im Test
ein Satz (F4a.8). Fixtures werden aus dem Erzeuger gebaut oder gegen ihn geprüft, Schlüssel
**und** Wertform.

### 10 · Eine Zahl, die aus einem Bestand kommt, gehört ins Archiv dieses Bestands, nicht in den Code

**0.66.3 (Michael-Befund):** `STEERING_ANCHOR_W` und `STEERING_ANCHOR_DATE` waren Johannes'
Startwerte und sein Stichtag — im Code. Ein zweiter Athlet sah eine fremde Vorgabe und einen
Kalender, in dem seine Einheiten nicht zählten. Die Klasse heißt **„für einen Athleten gebaut"**,
und sie hat Verwandte, die keine Vorgabe sind: `LEGACY_STEERING_ANCHOR` (nur noch Übernahme),
`fatigue_v2.BRIDGE_*` und `DECLINE_PER_HOUR` (Bestandszahlen des ersten Athleten, ungezeichnet
in der v2-Kette), `SETTINGS_NOTE` mit „an 13 Fahrten belegt" (Kacheltext), Kommentare mit
Bestandsdaten. Die Regel: Startwerte, Stichtage, Basislinien, Belegungszahlen entstehen aus dem
Archiv des Athleten und stehen dort. Was aus einem Bestand in den Code kommt, ist eine Setzung
und heißt so — mit dem Namen des Bestands daneben. **Der Wächter:** `test_zweiter_athlet.py`, ein
erfundener zweiter Athlet, in dessen Payloads keine Zahl des ersten vorkommen darf.

**Und die Auflage für jede Reparatur (seit 22.09.):** jede bekommt eine Prüfung, die **vorher**
an der unreparierten Fassung fällt — nicht nachher grün, sondern vorher rot. Das ist der einzige
Nachweis, dass sie das Richtige prüft.
